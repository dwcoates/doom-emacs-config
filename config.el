...
(defun +dwc/switch-to-project-buffer ()
  ...
  (interactive)
  (let* ((buffers (agent-repl--non-agent-buffers (projectile-project-buffer-names)))
         (candidates
          (mapcar (lambda (name)
...
(defun +dwc/toggle-last-buffer ()
  "Toggle between the last two buffers, excluding Agent REPL and minibuffers."
  (interactive)
  (let ((buf (car (agent-repl--non-agent-buffers
