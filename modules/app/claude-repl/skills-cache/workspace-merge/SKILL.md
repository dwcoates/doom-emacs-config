...
Do NOT attempt to merge the workspaces yourself in any way — **with the explicit carve-outs below**. The handling of the merge is EXCLUSIVELY the responsibility and right of downstream consumers. Your EXCLUSIVE job is to generate the aforementioned JSON file, and NOTHING else. **Carve-outs**: (1) when the current workspace is one of the named merge targets and the merge target branch is **not** master, it MUST be rebased onto the merge target branch before dispatch (see step 2) — the goal is that downstream consumers receive conflict-free commits and do not have to reason about conflicts themselves; (2) when the current workspace is one of the named merge targets and the merge target branch **is** master, a PR must be created and added to the merge queue before dispatch (see step 2M); (3) gns-sockets subscriptions bound to the current Claude session MUST be closed before the dispatch file is written when the current workspace is one of the named merge targets (see step 3). Without the sockets cleanup, the downstream editor tears down the workspace while leaving live subscriptions wired to a dead session, dangling the daemon's state and any associated hooks.
...
2. **Rebase onto the merge target, or create a PR — current workspace only.** Skip this step entirely if the current workspace is NOT among the named merge targets identified in step 1.
   ...
   **2M. If the merge target branch IS `master`** — create PR and add to merge queue (skip 2b/2c entirely):
   ...
   i. Create the PR with the `patch` and `self-certified` labels:
      ```bash
      gh pr create --fill --label patch --label self-certified
      ```
      - EXIT CODE 0: note the PR URL printed to stdout. Continue to step 2M-ii.
      - Non-zero: surface the error and STOP — do not proceed to step 3 or beyond.

   ii. Add the PR to the merge queue:
      ```bash
      gh pr merge --auto --merge
      ```
      - EXIT CODE 0: continue to step 3.
      - Non-zero: surface the error and STOP.
...
5. **Tell the user** which workspaces were targeted for merge, and (if step 3 printed any subscription ids on stdout) which subscriptions were closed. If step 2M ran, also mention the PR URL and that the workspace will be revived automatically if the PR is closed without merging.
...
- **CRITICAL: The only permitted mutating git operations are** the rebase in step 2b and its associated `git add` / `git rebase --continue` / `git rebase --abort` operations. No other mutating git commands are permitted — no resets, checkouts that discard work, pushes, or any other git mutations.
- **CRITICAL: the only side effects this skill performs are** the optional rebase/PR-creation in step 2, and the gns-sockets cleanup in step 3. Everything else either reads state or writes the single dispatch JSON via `run.sh --emit-commands`. Do not add additional mutating effects.
- **The downstream handler for master-target merges** (`refresh-master-from-origin`) will poll the PR asynchronously after the workspace is closed. No further action is required from the skill — the downstream handles both the success path (fetch+fast-forward master) and the failure path (revive workspace) automatically.
