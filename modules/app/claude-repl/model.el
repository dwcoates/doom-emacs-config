...
;; `claude-repl-model-scan-bytes' of the file when mtime changes.  Each
...
;; `claude-repl--ai-title-mtime') since both segments resolve the exact
...
(defcustom claude-repl-model-enabled t
  "Non-nil to render the Claude model in the vterm mode-line."
  :type 'boolean
  :group 'claude-repl)

(defcustom claude-repl-model-scan-bytes 32768
...
(defun claude-repl--model-prettify (model)
...
(defun claude-repl--model-extract-from-tail (tail)
...
(defun claude-repl--model-read-from-jsonl (path)
  "Return the most recent assistant model string from PATH, or nil.
Reads only the trailing `claude-repl-model-scan-bytes' so this stays
...
when no assistant entry is present in the scanned tail."
  (when (and path (file-readable-p path))
    (let* ((size (or (file-attribute-size (file-attributes path)) 0))
           (cap claude-repl-model-scan-bytes)
           (start (max 0 (- size cap))))
      (when (> size 0)
        (with-temp-buffer
          (insert-file-contents path nil start size)
          (claude-repl--model-extract-from-tail (buffer-string)))))))

;;;; Cached lookup

(defun claude-repl--model-for-ws (ws)
...
model is available."
  (let* ((path (claude-repl--ai-title-jsonl-path ws))
         ...
         (cache (claude-repl--ws-get ws :model-cache)))
    (cond
     ((null path) nil)
     ((null mtime) nil)
     ((and (consp cache)
      ...
      (nth 2 cache))
     (t
      (let ((model (claude-repl--model-read-from-jsonl path)))
...
model is yet available."
  (if (not claude-repl-model-enabled)
      ""
    (let ((ws (claude-repl--buffer-owner (current-buffer))))
      (if (not ws)
          ""
        (let ((label (claude-repl--model-prettify (claude-repl--model-for-ws ws))))
...
(defconst claude-repl--model-mode-line-spec
...
(defun claude-repl--model-attach-to-mode-line (buf)
  ...
  (when (buffer-live-p buf)
    (with-current-buffer buf
      (when (and (listp mode-line-format)
                 (not (member claude-repl--model-mode-line-spec
                              mode-line-format)))
        (setq-local mode-line-format
                    (append mode-line-format
                            (list claude-repl--model-mode-line-spec)))
        (force-mode-line-update t)))))

(defun claude-repl-model-attach-all ()
     ...
     (lambda (_ws plist)
       (let ((buf (plist-get plist :vterm-buffer)))
         (when (and buf (buffer-live-p buf))
           (claude-repl--model-attach-to-mode-line buf))))
     claude-repl--workspaces)))

(claude-repl-model-attach-all)

(provide 'claude-repl-model)
;;; model.el ends here
