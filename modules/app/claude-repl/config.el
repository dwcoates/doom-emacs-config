      ...
      (let ((add-fn
        ...
        (if fork-session-id
          ...
          (claude-repl--async-git
           ...
           (lambda (_ok output)
             (claude-repl--log "worktree fetch: %s" output)
             (funcall add-fn))))))))
