...
The number of lines hidden is controlled by `claude-repl--hide-overlay-line-count'."
  (if (<= (point-max) 1)
      (progn
        (claude-repl--log-verbose (claude-repl--ws-current-name) "hide-overlay-region: nil (buffer empty)")
        nil)
    (let* ((end (point-max))
      ...
      (if (< start end)
          (progn
            (claude-repl--log-verbose (claude-repl--ws-current-name) "hide-overlay-region: start=%d end=%d" start end)
            (cons start end))
        (claude-repl--log-verbose (claude-repl--ws-current-name) "hide-overlay-region: nil (degenerate region start=%d end=%d)" start end)
        nil))))
...
The region is computed by `claude-repl--hide-overlay-region'."
  (if (not claude-repl-hide-input-box)
      (claude-repl--log-verbose (claude-repl--ws-current-name) "create-hide-overlay: skipped (hide-input-box nil)")
    (let ((region (claude-repl--hide-overlay-region)))
      (if (not region)
          (claude-repl--log-verbose (claude-repl--ws-current-name) "create-hide-overlay: skipped (no region)")
        ...
        (claude-repl--log-verbose (claude-repl--ws-current-name) "create-hide-overlay: created start=%d end=%d" (car region) (cdr region))))))
...
(defun claude-repl--delete-hide-overlay ()
  ...
  (if (and claude-repl-hide-overlay
      ...
      (progn
        (claude-repl--log-verbose (claude-repl--ws-current-name) "delete-hide-overlay: deleted")
        (delete-overlay claude-repl-hide-overlay))
    (claude-repl--log-verbose (claude-repl--ws-current-name) "delete-hide-overlay: no overlay to delete"))
...
Otherwise looks up the vterm buffer for the current workspace."
  (if (claude-repl--claude-buffer-p)
      (progn
        (claude-repl--log-verbose (claude-repl--ws-current-name) "resolve-overlay-target-buffer: using current claude buffer")
        (current-buffer))
...
Resolves the target buffer, then deletes and recreates the overlay in it."
  (let ((buf (claude-repl--resolve-overlay-target-buffer)))
    (if (not (and buf (buffer-live-p buf)))
        (claude-repl--log-verbose (claude-repl--ws-current-name) "update-hide-overlay: skipped (buf nil or dead)")
      (claude-repl--log-verbose (claude-repl--ws-current-name) "update-hide-overlay: updating buf=%s" (buffer-name buf))
...
Uses reference counting so multiple sessions don't clobber each other."
  (claude-repl--log (claude-repl--ws-current-name) "enable-hide-overlay refcount=%d" claude-repl--hide-overlay-refcount)
...
(defun claude-repl--vterm-redraw-advice (&rest _)
  ...
  (if claude-repl--in-redraw-advice
      (claude-repl--log-verbose (claude-repl--ws-current-name) "vterm-redraw-advice: skipped (recursion guard)")
    (let ((claude-repl--in-redraw-advice t))
      (if (not (claude-repl--claude-buffer-p))
          (claude-repl--log-verbose (claude-repl--ws-current-name) "vterm-redraw-advice: skipped (non-claude buffer)")
...
session releases it."
  ...
  (claude-repl--log (claude-repl--ws-current-name) "disable-hide-overlay refcount=%d" claude-repl--hide-overlay-refcount)
  (when (zerop claude-repl--hide-overlay-refcount)
...
(defun claude-repl-toggle-hide-input-box ()
  ...
  (interactive)
  (claude-repl--log (claude-repl--ws-current-name) "toggle-hide-input-box -> %s" (not claude-repl-hide-input-box))
    ...
    (if (and (claude-repl--default-background-request-p index args)
        ...
        (let ((override (claude-repl--grey-hex claude-repl--vterm-background-grey)))
          ...
          (when (eq claude-repl-debug 'verbose)
            (claude-repl--log-verbose (claude-repl--ws-current-name) "vterm-color-advice: claude buffer background override color=%s" override))
          override)
      result)))
