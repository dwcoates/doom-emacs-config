...
(defcustom claude-repl-command-prefix
  (concat "\n"
          ...
          "* TLDR spec: "
          "\n"
          "  - Structure is a handful of top-level emoji-prefixed bullets that give the several high-level ideas of the response, "
          "with each top-level bullet fleshed out a bit more via subbullets that zoom in on that idea. "
          "\n"
          "  - The number of top-level bullets MUST be capped at 3 — additional detail belongs in subbullets (recursively) of those at-most-3 top-level bullets, "
          "since the top level is for the high-level vision of the response and not for enumerating every point. "
          "\n"
          "  - The per-entry subbullet zoom-in IS the resolution detail — do NOT emit any additional 'even shorter' or 'recursive' TLDR section. "
          "\n"
          "  - One sentence per bullet, both top-level and subbullet. "
          "\n"
          "  - TLDR bullets MUST never contain emdashes or semicolons under any circumstances, since each is a sign that additional information should instead be pushed into a (recursively-nested) subbullet of the bullet. "
          "\n"
          ...
          "\n"
          "  - Use of subbullets (recursively, at any depth) is the ONLY permissible way to attach additional or qualifying information to a TLDR bullet, and second sentences inside a single bullet are never allowed. "
          "\n"
          "  - Each bullet (top-level and subbullet) MUST make clear via its implicit language whether it describes "
          ...
          "\n"
          "  - Each top-level bullet MUST be prefixed with a relevant prefixing emoji for top-level bullets (subbullets are not emoji-prefixed). "
          "\n"
