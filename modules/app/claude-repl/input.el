...
(defcustom claude-repl-command-prefix "ALLOWED & ENCOURAGED: create new commits freely for atomic, well-scoped units of work — do not ask first; ensure applicable tests run and pass before each commit. NOT ALLOWED without my EXPLICIT, per-use permission: other mutating git commands (push, rebase, pull, merge, reset, checkout that discards work, force-push, branch deletion); installing or uninstalling packages/tools; operating on files outside the current project. OTHER: I will NEVER ask a rhetorical question -- if I ask 'why does X happen?' or 'is Y broken?' do not infer that I want a fix; just answer. Take action only when explicitly requested. Other than the standing commit authorization, take only actions that follow directly from what I explicitly request in this prompt."
