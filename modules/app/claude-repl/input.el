...
(defcustom claude-repl-command-prefix
  (concat "\n"
          ...
          "enumeration of issues, etc). Even when giving a long answer for an analysis effort, STILL give a very short, terse TLDR version at the "
          "end (as though it were a simple short-answer request). Make heavy use of bullets in the TLDR, use them aggressively to structure the TLDR "
          ...
          "Always include a brief TLDR summary of changes as you would for analysis. "
          "Regardless of the response (unless the response is 1-5 sentences long), ALWAYS provide a brief 'TLDR' section as the last thing in the final "
          " response. Always. "
          "\n"
          "* After the TLDR section, ALWAYS include a 'TLDR's TLDR' section as the VERY LAST item in the response. "
          "The 'TLDR's TLDR' is a TLDR of the TLDR itself, following the exact same specification as the TLDR (heavy use of bullets, "
          ...
          "but applied recursively to the TLDR content. The same exception applies: skip it only when the response is 1-5 sentences long. "
          "This is non-negotiable and must always be the final section of any response that has a TLDR. "
          "\n"
