...
(defcustom claude-repl-command-prefix
  (concat "\n"
          ...
          "* TLDR spec: "
          "\n"
          "  - The Response TLDR MUST be rendered as a MECE numbered ASCII tree at depth 3 (root branch, mid-level branch, leaf), using ASCII box-drawing connectors (├──, └──, │) for the parent-child edges and dotted hierarchical numbering for the labels (e.g., '1. ...', '1.1. ...', '1.1.1. ...'). "
          "\n"
          "  - Depth 3 is non-negotiable: the tree MUST NOT be flattened to depth 2 or lower, and if you think a lower depth fits the response better you are wrong — instead, justify why each leaf at depth 3 makes sense, since the leaves are part of the response's ideal-state coverage. "
          "\n"
          ...
          "\n"
          "  - Each TLDR node (root, mid-level branch, leaf) IS the resolution detail at its level, so the tree replaces any 'even shorter' or 'recursive' TLDR section — leaves are where the resolution finally lands. "
          "\n"
          "  - One sentence per bullet, at every depth of the tree (root, mid-level branch, and leaf). "
          "\n"
          "  - TLDR bullets MUST never contain emdashes or semicolons under any circumstances, since each is a sign that additional information should instead be pushed into a (recursively-nested) subbullet of the bullet, up to the depth-3 cap. "
          "\n"
          ...
          "\n"
          "  - Use of subbullets (recursively, at any depth up to the depth-3 cap) is the ONLY permissible way to attach additional or qualifying information to a TLDR bullet, and second sentences inside a single bullet are never allowed. "
          "\n"
          "  - Each bullet (root branch, mid-level branch, and leaf) MUST make clear via its implicit language whether it describes "
          ...
          "\n"
          "  - Each root branch (depth-1 node) MUST be prefixed with a relevant prefixing emoji for top-level bullets, placed immediately after its numeric label (e.g., '1. 🔧 ...'); mid-level branches and leaves are NOT emoji-prefixed. "
          "\n"
