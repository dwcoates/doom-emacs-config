...
(defcustom claude-repl-command-prefix
  (concat "\n"
          ...
          "instead add it as a subbullet of the bullet. Always use subbullets instead of semicolons or emdashes. Also include a relevant prefixing emoji for the top-level bullets. "
          "Always include a brief 'last response TLDR' summary of changes as you would for analysis. "
          "Regardless of the response (unless the response is 1-5 sentences long), ALWAYS provide a brief 'last response TLDR' section near the end of the final "
          "response. Always. "
          "Every TLDR section ('large context TLDR', 'last response TLDR', and 'TLDR's TLDR') is HARD-CAPPED at 5 top-level bullets MAXIMUM. "
          "If you find yourself wanting more than 5 top-level bullets in any TLDR, you MUST cut, merge, or drop items until at most 5 remain — "
          "do NOT attempt to circumvent this cap by making the surviving bullets individually longer, denser, or by stuffing extra "
          "content into subbullets to compensate. Each top-level bullet remains a single short sentence, and subbullets remain bound "
          "to the same one-sentence rule as before; the 5-item cap is a hard ceiling on top-level information density, not a budget "
          "to redistribute. If material genuinely cannot fit, it does not belong in the TLDR. "
          "\n"
          "* BEFORE the 'last response TLDR' section, ALWAYS include a 'large context TLDR' section that summarizes the broader context of the "
          "conversation/task so far -- not just the current response, but the arc of the whole conversation/task: what we've been working on, "
          "what's been decided, the current state of things, and any outstanding items. This gives the reader a recap of the bigger picture "
          "before drilling into the latest turn. It follows the EXACT same format specification as the 'last response TLDR' (heavy use of "
          "bullets, subbullets instead of semicolons/emdashes/second-sentences, relevant prefixing emoji for top-level bullets, one sentence per bullet). "
          "The same exception applies: skip it only when the response is 1-5 sentences long. "
          "The ordering of trailing sections is therefore: (1) 'large context TLDR' first, (2) 'last response TLDR' second, (3) 'TLDR's TLDR' third, (4) 'Summary' last. "
          "\n"
          "* After the 'last response TLDR' section, ALWAYS include a 'TLDR's TLDR' section directly before the final 'Summary' section. "
          "The 'TLDR's TLDR' is a TLDR of the 'last response TLDR' itself, following the exact same specification as the TLDR (heavy use of bullets, "
          "subbullets instead of semicolons/emdashes/second-sentences, relevant prefixing emoji for top-level bullets, one sentence per bullet) "
          "but applied recursively to the 'last response TLDR' content (i.e. a TLDR of the TLDR). The same exception applies: skip it only when the "
          "response is 1-5 sentences long. "
          "The same 5-top-level-bullet hard cap and no-bypass-by-lengthening rule applies recursively to the 'TLDR's TLDR'. "
          "\n"
          "* FINAL directive: ALWAYS end every response with a 'Summary' section as the VERY LAST item in the response. "
          "The 'Summary' is exactly ONE SENTENCE that summarizes the response. No bullets, no subbullets, no lists, no emoji, no additional formatting -- "
          "just a single plain sentence. This is non-negotiable and must always be the final section of every response, with no exceptions "
          "(it applies even to 1-5 sentence responses that skip the TLDR sections). "
          "\n"
