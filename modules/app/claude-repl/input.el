...
(defcustom claude-repl-command-prefix
  (concat "\n"
          ...
          "\n"
          "* Regardless of the response (unless the response is 1-5 sentences long), ALWAYS provide THREE TLDR sections at the very end of the response, "
          "in this strict order: "
          "\n"
          "  1. 'Convo TLDR' — a TLDR of the ENTIRE conversation so far (not just this response), following the TLDR spec below. "
          "\n"
          "  2. 'Response TLDR' — a TLDR of THIS response specifically, following the TLDR spec below. "
          "\n"
          "  3. 'Response TLDR's TLDR' — a TLDR of the Response TLDR itself, applied recursively to the Response TLDR content, following the TLDR spec below. "
          "This must be the VERY LAST section of the response. "
          "\n"
          "* TLDR spec (applies to all three sections): heavy use of bullets, subbullets instead of semicolons/emdashes/second-sentences, "
          "relevant prefixing emoji for top-level bullets, one sentence per bullet. "
          "Each TLDR section's header MUST indicate whether changes were made in this response, in parentheses immediately after the section title "
          "(e.g., 'Convo TLDR (changes made)' or 'Response TLDR (no changes made)'). "
          ...
          "The annotation must be one of exactly 'changes made' or 'no changes made' — no other wording. "
          "The 1-5 sentence exception applies to all three sections uniformly: skip all three only when the response is 1-5 sentences long. "
          "The order Convo TLDR -> Response TLDR -> Response TLDR's TLDR is non-negotiable and must always be the final block of any response that has TLDRs. "
          "\n"
