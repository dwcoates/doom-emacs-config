...
;;
;; The close routes through `claude-repl--simple-hide-and-preserve-status'
...
(defcustom claude-repl-close-panels-on-open-fns
  '(switch-to-buffer
    pop-to-buffer-same-window
    find-file)
  "Functions that should close visible Claude panels before opening.
Each is advised :before with `claude-repl--close-panels-before-open'.
...
(defvar claude-repl--closing-panels-for-open nil
  "Non-nil while `claude-repl--close-panels-before-open' is closing panels.
...
(defun claude-repl--open-target-is-panel-p (args)
...
machinery uses to place panels into their own windows."
  (let* ((arg (car args))
    ...
    (and buf (buffer-live-p buf) (claude-repl--claude-panel-buffer-p buf))))

(defun claude-repl--close-panels-before-open (&rest args)
  "Close Claude REPL panels before an interactive buffer/file open.
Installed as :before advice on `claude-repl-close-panels-on-open-fns'.
No-op unless both panels are currently visible and the open target is
not itself a Claude panel buffer (see
`claude-repl--open-target-is-panel-p').  Routes through
`claude-repl--simple-hide-and-preserve-status' (the `SPC o c' close)
...
close leaves selected."
  (unless claude-repl--closing-panels-for-open
    (when (and (claude-repl--panels-visible-p)
               (not (claude-repl--open-target-is-panel-p args)))
      (let ((claude-repl--closing-panels-for-open t))
        (claude-repl--simple-hide-and-preserve-status)))))

(dolist (fn claude-repl-close-panels-on-open-fns)
  (advice-add fn :before #'claude-repl--close-panels-before-open))
