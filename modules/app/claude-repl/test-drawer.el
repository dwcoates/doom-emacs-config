...
;;;; ---- Tests: recenter-if-offscreen ----

(defmacro claude-repl-drawer-test--with-displayed-drawer (lines &rest body)
  "Create the singleton drawer buffer, fill with LINES numbered lines,
display in `selected-window', and execute BODY with both `buf' and
`win' bound.  Cleans up the buffer on exit."
  (declare (indent 1))
  `(let ((buf (get-buffer-create claude-repl-drawer-buffer-name))
         (win (selected-window)))
     (unwind-protect
...
(ert-deftest claude-repl-drawer-test-recenter-if-offscreen-centers-when-below ()
  "Helper recenters the drawer window when point sits past `window-end'.
Asserts `window-start' advances toward point so the cursor line moves
off the bottom edge into the middle.  Uses `window-start' inequality
rather than `pos-visible-in-window-p' because the latter requires real
redisplay and returns nil unconditionally under `-batch'."
  (claude-repl-test--with-clean-state
    (claude-repl-drawer-test--with-displayed-drawer 500
      (with-selected-window win
        ...
        (let ((start-before (window-start win))
          ...
          ;; And the new window-start must not have overshot point — point
          ;; has to be at or after window-start for the cursor line to be
          ;; on-screen at all.
          (should (<= (window-start win) pt)))))))

(ert-deftest claude-repl-drawer-test-recenter-if-offscreen-centers-when-above ()
  "Helper also handles the upward case — point above `window-start'.
Anchors `window-start' near the bottom, moves point to `point-min',
asserts the helper rewinds `window-start' toward point."
  (claude-repl-test--with-clean-state
    (claude-repl-drawer-test--with-displayed-drawer 500
      (with-selected-window win
        (let ((bottom-start (with-current-buffer buf
                              (save-excursion
                                (goto-char (point-max))
                                (forward-line -2)
                                (point)))))
          (set-window-start win bottom-start)
          (with-current-buffer buf (goto-char (point-min)))
          (set-window-point win (point-min))
          (claude-repl-drawer--recenter-if-offscreen buf)
          (should (< (window-start win) bottom-start)))))))

(ert-deftest claude-repl-drawer-test-recenter-if-offscreen-noop-when-visible ()
  "When point is already on-screen, the helper must not call `recenter'.
Stubs `pos-visible-in-window-p' to return t (simulating an on-screen
point) and asserts `recenter' is never invoked — verifies the short-
circuit contract regardless of actual frame geometry.  Real
`pos-visible-in-window-p' is unreliable under `-batch' (always nil),
so the test forces the visibility branch with a stub."
  (claude-repl-test--with-clean-state
    (claude-repl-drawer-test--with-displayed-drawer 500
      (with-selected-window win
        (set-window-start win (point-min))
...
(ert-deftest claude-repl-drawer-test-recenter-if-offscreen-noop-when-no-window ()
  "Helper is a no-op (no error) when the drawer buffer has no live window."
  (claude-repl-test--with-clean-state
    (let ((buf (get-buffer-create claude-repl-drawer-buffer-name)))
      (unwind-protect
          ;; Buffer exists but is not displayed anywhere → helper should
          ;; iterate zero windows and return cleanly.
          (should-not (claude-repl-drawer--recenter-if-offscreen buf))
        (when (buffer-live-p buf) (kill-buffer buf))))))

(ert-deftest claude-repl-drawer-test-recenter-if-offscreen-noop-when-no-buffer ()
  "Helper is a no-op (no error) when no drawer buffer exists at all.
Default-arg path: `when-let*' on `(get-buffer ...)' short-circuits."
  (claude-repl-test--with-clean-state
    (when-let ((b (get-buffer claude-repl-drawer-buffer-name)))
      (kill-buffer b))
    (should-not (claude-repl-drawer--recenter-if-offscreen))))

(ert-deftest claude-repl-drawer-test-post-command-calls-recenter ()
  "`--post-command' must invoke `--recenter-if-offscreen' so navigation
that crosses a scroll boundary triggers the center-on-screen behavior."
  (claude-repl-test--with-clean-state
    (let ((called nil))
      (cl-letf (((symbol-function 'claude-repl-drawer--recenter-if-offscreen)
