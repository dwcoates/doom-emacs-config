...
### `agent-repl--notify-backend-terminal-notifier (title message)`
...
### `agent-repl--notify-backend-osascript (title message)`
...
| # | Edge Case | Test Status |
|---|-----------|-------------|
| 1 | `terminal-notifier` is found -- returns `#'agent-repl--notify-backend-terminal-notifier` | Covered |
| 2 | `terminal-notifier` is NOT found -- falls back to `#'agent-repl--notify-backend-osascript` | Covered |
| 3 | Both backends unavailable (osascript is always the fallback, never checked) | Implicitly covered by case 2 |

### `agent-repl--notification-backend` (defvar)
Variable set at load time by calling `agent-repl--select-notification-backend`.
...
4. **Backend function signaling an error** -- `agent-repl--notify` has no error handling; this is a design choice (let errors propagate), not a bug.
