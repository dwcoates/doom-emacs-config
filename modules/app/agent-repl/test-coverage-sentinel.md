...
### `agent-repl--ws-for-dir-fast` (line 22)
...
### `agent-repl--ws-for-dir`
Delegates to the git-root fast path. The Docker-sandbox container-path
fallback it used to combine with was deleted with the sandbox feature.

| # | Edge Case | Covered | Test Name |
|---|-----------|---------|-----------|
| 1 | Fast path succeeds | YES | `test-ws-for-dir-returns-fast-path` |
| 2 | Fast path misses, returns nil | YES | `test-ws-for-dir-returns-nil-when-fast-path-misses` |

### `agent-repl--read-sentinel-file` (line 71)
...
### `agent-repl--process-sentinel-file` (line 89)
...
### `agent-repl--on-permission-event` (line 114)
...
### `agent-repl--on-stop-event` (line 119)
...
### `agent-repl--on-prompt-submit-event` (line 127)
...
### `agent-repl--sentinel-dispatch-alist` (constant, line 133)
...
### `agent-repl--dispatch-sentinel-file` (line 149)
...
### `agent-repl--dispatch-sentinel-event` (line 161)
...
### `agent-repl--poll-workspace-notifications` (line 177)
...
4. **`poll-workspace-notifications` hidden file exclusion**: Verify the regex `\\`[^.]` correctly excludes dotfiles.
5. **`on-stop-event` when status is not :thinking**: Verify `ws-clear` is still called (it's a clear-if-status, so it would be a no-op).
