;;; test-overlay.el --- ERT tests for agent-repl overlay system -*- lexical-binding: t; -*-
...
;;;; ---- Tests: agent-repl--hide-overlay-region ----

(ert-deftest agent-repl-test-overlay-region-empty-buffer ()
  "In an empty buffer (point-max = 1), `hide-overlay-region' returns nil."
  (agent-repl-test--with-temp-buffer " *test-region-empty*"
    (should-not (agent-repl--hide-overlay-region))))

(ert-deftest agent-repl-test-overlay-region-single-char ()
  "A buffer with a single character should return a valid region."
  (agent-repl-test--with-temp-buffer " *test-region-single*"
    (insert "x")
    (let ((region (agent-repl--hide-overlay-region)))
      (should region)
      (should (< (car region) (cdr region))))))

(ert-deftest agent-repl-test-overlay-region-fewer-lines-than-count ()
  "When the buffer has fewer lines than `hide-overlay-line-count', region covers from beginning."
  (agent-repl-test--with-temp-buffer " *test-region-few*"
    (insert "line1\nline2\n")
    (let ((region (agent-repl--hide-overlay-region)))
      (should region)
...
(ert-deftest agent-repl-test-overlay-region-exact-line-count ()
  "When the buffer has exactly `hide-overlay-line-count' lines, region covers them."
  (agent-repl-test--with-temp-buffer " *test-region-exact*"
...
(ert-deftest agent-repl-test-overlay-region-more-lines-than-count ()
  "When the buffer has more lines than `hide-overlay-line-count', region starts after the visible lines."
  (agent-repl-test--with-temp-buffer " *test-region-more*"
...
;;;; ---- Tests: agent-repl--create-hide-overlay ----

(ert-deftest agent-repl-test-create-overlay-respects-flag ()
  "When `agent-repl-hide-input-box' is nil, `create-hide-overlay' should NOT create an overlay."
  (agent-repl-test--with-temp-buffer " *test-no-overlay*"
    (let ((agent-repl-hide-input-box nil))
      (insert "some content\nline two\nline three\nline four\nline five\n")
      (agent-repl--create-hide-overlay)
      (should-not agent-repl-hide-overlay))))

(ert-deftest agent-repl-test-create-overlay-creates-when-enabled ()
  "When `agent-repl-hide-input-box' is t and buffer has content, overlay is created."
  (agent-repl-test--with-temp-buffer " *test-yes-overlay*"
    (let ((agent-repl-hide-input-box t))
      (insert "line1\nline2\nline3\nline4\nline5\nline6\n")
      (agent-repl--create-hide-overlay)
      (should agent-repl-hide-overlay)
      (should (overlayp agent-repl-hide-overlay)))))

(ert-deftest agent-repl-test-create-overlay-empty-buffer ()
  "In an empty buffer, `create-hide-overlay' should not create an overlay even when enabled."
  (agent-repl-test--with-temp-buffer " *test-create-empty*"
    (let ((agent-repl-hide-input-box t))
      (agent-repl--create-hide-overlay)
      (should-not agent-repl-hide-overlay))))

(ert-deftest agent-repl-test-create-overlay-has-display-property ()
  "The created overlay should have a 'display property set to empty string."
  (agent-repl-test--with-temp-buffer " *test-overlay-display*"
    (let ((agent-repl-hide-input-box t))
      (insert "line1\nline2\nline3\nline4\nline5\n")
      (agent-repl--create-hide-overlay)
      (should agent-repl-hide-overlay)
      (should (equal (overlay-get agent-repl-hide-overlay 'display) "")))))

(ert-deftest agent-repl-test-create-overlay-has-evaporate-property ()
  "The created overlay should have the 'evaporate property set to t."
  (agent-repl-test--with-temp-buffer " *test-overlay-evaporate*"
    (let ((agent-repl-hide-input-box t))
      (insert "line1\nline2\nline3\nline4\nline5\n")
      (agent-repl--create-hide-overlay)
      (should agent-repl-hide-overlay)
      (should (overlay-get agent-repl-hide-overlay 'evaporate)))))

;;;; ---- Tests: agent-repl--delete-hide-overlay ----

(ert-deftest agent-repl-test-delete-overlay-noop-when-nil ()
  "When `agent-repl-hide-overlay' is nil, `delete-hide-overlay' should not error."
  (agent-repl-test--with-temp-buffer " *test-del-nil*"
    (setq-local agent-repl-hide-overlay nil)
    ;; Should not signal any error
    (agent-repl--delete-hide-overlay)
    (should-not agent-repl-hide-overlay)))

(ert-deftest agent-repl-test-delete-overlay-deletes-existing ()
  "When an overlay exists, `delete-hide-overlay' should remove it and nil the variable."
  (agent-repl-test--with-temp-buffer " *test-del-existing*"
    (let ((agent-repl-hide-input-box t))
      (insert "line1\nline2\nline3\nline4\nline5\n")
      (agent-repl--create-hide-overlay)
      (should agent-repl-hide-overlay)
      (let ((ov agent-repl-hide-overlay))
        (agent-repl--delete-hide-overlay)
        (should-not agent-repl-hide-overlay)
        ;; The overlay should no longer be attached to a buffer
        (should-not (overlay-buffer ov))))))

(ert-deftest agent-repl-test-delete-overlay-handles-already-deleted ()
  "If the overlay exists but its buffer is already killed, `delete-hide-overlay' should still nil the variable."
  (agent-repl-test--with-temp-buffer " *test-del-killed*"
    (let ((agent-repl-hide-input-box t))
      (insert "line1\nline2\nline3\nline4\nline5\n")
      (agent-repl--create-hide-overlay)
      (should agent-repl-hide-overlay)
      ...
      (should-not (overlay-buffer agent-repl-hide-overlay))
      ;; delete-hide-overlay should handle this gracefully
      (agent-repl--delete-hide-overlay)
      (should-not agent-repl-hide-overlay))))

;;;; ---- Tests: agent-repl--resolve-overlay-target-buffer ----
      ...
      (let ((fake-buf (get-buffer-create " *fake-vterm*")))
        (unwind-protect
            (cl-letf (((symbol-function '+workspace-current-name)
                       (lambda () "test-ws")))
              (agent-repl--ws-put "test-ws" :vterm-buffer fake-buf)
...
;;;; ---- Tests: agent-repl--update-hide-overlay ----

(ert-deftest agent-repl-test-update-overlay-nil-buffer ()
  "When resolve returns nil, `update-hide-overlay' should be a no-op."
  (agent-repl-test--with-clean-state
    (cl-letf (((symbol-function 'agent-repl--resolve-overlay-target-buffer)
               (lambda () nil)))
      ;; Should not error
      (agent-repl--update-hide-overlay))))

(ert-deftest agent-repl-test-update-overlay-dead-buffer ()
...
(ert-deftest agent-repl-test-update-overlay-recreates ()
  "update-hide-overlay deletes old overlay and creates a new one."
  (agent-repl-test--with-clean-state
    (agent-repl-test--with-temp-buffer " *test-update-recreate*"
      (let ((agent-repl-hide-input-box t))
        (insert "line1\nline2\nline3\nline4\nline5\nline6\n")
        (cl-letf (((symbol-function 'agent-repl--resolve-overlay-target-buffer)
                   (lambda () (current-buffer))))
          (agent-repl--update-hide-overlay)
          (let ((first-ov agent-repl-hide-overlay))
            (should first-ov)
...
(ert-deftest agent-repl-test-overlay-refcount ()
...
(ert-deftest agent-repl-test-enable-overlay-from-nonzero ()
  ...
  (agent-repl-test--with-clean-state
    (let ((advice-added 0))
      (cl-letf (((symbol-function 'advice-add)
        ...
        (agent-repl--enable-hide-overlay)
        (should (= advice-added 1))
        (should (= agent-repl--hide-overlay-refcount 3))))))

(ert-deftest agent-repl-test-disable-overlay-clamps-at-zero ()
...
(ert-deftest agent-repl-test-disable-overlay-cleans-up-overlay ()
  ...
  (agent-repl-test--with-clean-state
    (let ((delete-called 0))
      (cl-letf (((symbol-function 'advice-add)
                ...
                ((symbol-function 'advice-remove)
                 (lambda (&rest _) nil))
                ((symbol-function 'agent-repl--delete-hide-overlay)
                 (lambda () (cl-incf delete-called))))
        (agent-repl--enable-hide-overlay)
        (agent-repl--disable-hide-overlay)
        (should (= delete-called 1))))))

;;;; ---- Tests: agent-repl--vterm-redraw-advice ----

(ert-deftest agent-repl-test-redraw-advice-reentrancy-guard ()
    ...
    (let ((update-count 0))
      (agent-repl-test--with-temp-buffer "*claude-panel-abcd1234*"
        (cl-letf (((symbol-function 'agent-repl--update-hide-overlay)
          ...
          (let ((agent-repl--in-redraw-advice nil))
            (agent-repl--vterm-redraw-advice)
  ...
  (agent-repl-test--with-clean-state
    (agent-repl-test--with-temp-buffer "*claude-panel-test9999*"
      (cl-letf (((symbol-function 'agent-repl--update-hide-overlay)
        ...
        (agent-repl--vterm-redraw-advice 'arg1 'arg2 'arg3)))))

(ert-deftest agent-repl-test-redraw-advice-sets-reentrancy-flag ()
      ...
      (agent-repl-test--with-temp-buffer "*claude-panel-f1a9c4ec*"
        (cl-letf (((symbol-function 'agent-repl--update-hide-overlay)
          ...
          (let ((agent-repl--in-redraw-advice nil))
            ...
            (should-not agent-repl--in-redraw-advice)))))))

;;;; ---- Tests: agent-repl-toggle-hide-input-box ----

(ert-deftest agent-repl-test-toggle-enables-when-disabled ()
  "Toggling when disabled should set `agent-repl-hide-input-box' to t."
  (agent-repl-test--with-clean-state
    (let ((agent-repl-hide-input-box nil)
          (update-called nil))
      (cl-letf (((symbol-function 'agent-repl--update-hide-overlay)
                 (lambda () (setq update-called t))))
        (agent-repl-toggle-hide-input-box)
        (should agent-repl-hide-input-box)
        (should update-called)))))

(ert-deftest agent-repl-test-toggle-disables-when-enabled ()
  "Toggling when enabled should set `agent-repl-hide-input-box' to nil."
  (agent-repl-test--with-clean-state
    (let ((agent-repl-hide-input-box t))
      (cl-letf (((symbol-function 'agent-repl--update-hide-overlay)
                 (lambda () nil)))
        (agent-repl-toggle-hide-input-box)
        (should-not agent-repl-hide-input-box)))))

(ert-deftest agent-repl-test-toggle-calls-update ()
...
(ert-deftest agent-repl-test-hide-overlay-line-count-is-positive ()
  "The line count constant should be a positive integer."
  (should (integerp agent-repl--hide-overlay-line-count))
  (should (> agent-repl--hide-overlay-line-count 0)))

(ert-deftest agent-repl-test-in-redraw-advice-declared ()
  "`agent-repl--in-redraw-advice' should be `boundp'."
  (should (boundp 'agent-repl--in-redraw-advice)))

;;;; ---- Tests: Remaining edge cases ----

(ert-deftest agent-repl-test-overlay-region-narrowed-buffer ()
  "In a narrowed buffer, `hide-overlay-region' respects narrowing boundaries."
  (agent-repl-test--with-temp-buffer " *test-region-narrowed*"
    (insert "line1\nline2\nline3\nline4\nline5\nline6\nline7\nline8\n")
...
(ert-deftest agent-repl-test-create-overlay-front-advance ()
  "The created overlay should have front-advance set to t."
  (agent-repl-test--with-temp-buffer " *test-overlay-front-advance*"
    (let ((agent-repl-hide-input-box t)
...
(ert-deftest agent-repl-test-create-overlay-twice-leaks ()
  "Calling `create-hide-overlay' twice without deleting leaks the first overlay."
  (agent-repl-test--with-temp-buffer " *test-create-twice*"
    (let ((agent-repl-hide-input-box t))
      (insert "line1\nline2\nline3\nline4\nline5\n")
      (agent-repl--create-hide-overlay)
      (let ((first-ov agent-repl-hide-overlay))
        ...
        (agent-repl--create-hide-overlay)
        (let ((second-ov agent-repl-hide-overlay))
...
(ert-deftest agent-repl-test-delete-overlay-non-overlay-value ()
  "When `agent-repl-hide-overlay' is non-nil but not an overlay, `delete-hide-overlay' signals an error."
  (agent-repl-test--with-temp-buffer " *test-del-non-overlay*"
    (setq-local agent-repl-hide-overlay "not-an-overlay")
    ;; overlay-buffer will error on a non-overlay argument
    (should-error (agent-repl--delete-hide-overlay) :type 'wrong-type-argument)))

(ert-deftest agent-repl-test-resolve-target-killed-vterm-buffer ()
  "When the workspace vterm-buffer is killed, resolve returns the killed buffer."
  (agent-repl-test--with-clean-state
    (agent-repl-test--with-temp-buffer "*scratch-killed-vterm*"
      (let ((killed-buf (get-buffer-create " *killed-vterm*")))
        (kill-buffer killed-buf)
        (cl-letf (((symbol-function '+workspace-current-name)
                   (lambda () "killed-ws")))
          (agent-repl--ws-put "killed-ws" :vterm-buffer killed-buf)
          (let ((result (agent-repl--resolve-overlay-target-buffer)))
            ;; Returns the killed buffer object (not nil)
            (should result)
            (should-not (buffer-live-p result))))))))

(ert-deftest agent-repl-test-toggle-is-interactive ()
  "`agent-repl-toggle-hide-input-box' should be an interactive command."
  (should (commandp #'agent-repl-toggle-hide-input-box)))

(ert-deftest agent-repl-test-toggle-displays-message ()
...
(ert-deftest agent-repl-test-grey-format ()
...
(ert-deftest agent-repl-test-vterm-background-grey-value ()
  "`agent-repl--vterm-background-grey' should be 15."
  (should (= agent-repl--vterm-background-grey 15)))

;;;; ---- Tests: set-buffer-background ----

(ert-deftest agent-repl-test-set-buffer-background-sets-faces ()
...
(ert-deftest agent-repl-test-set-buffer-background-correct-hex ()
...
(ert-deftest agent-repl-test-set-buffer-background-different-levels ()
...
(ert-deftest agent-repl-test-vterm-font-scale-default-greater-than-one ()
  "`agent-repl-vterm-font-scale' should default to an enlarging value."
  (should (> agent-repl-vterm-font-scale 1.0)))

(ert-deftest agent-repl-test-vterm-font-scale-default-value ()
  "`agent-repl-vterm-font-scale' should default to the enlarged 1.35 scale."
  (should (= agent-repl-vterm-font-scale 1.35)))

(ert-deftest agent-repl-test-apply-vterm-font-scale-remaps-default-height ()
  "apply-vterm-font-scale should remap the default face height by the scale."
  (agent-repl-test--with-temp-buffer " *test-font-scale*"
    (let ((remapped nil)
          (agent-repl-vterm-font-scale 1.4))
      (cl-letf (((symbol-function 'face-remap-add-relative)
                 (lambda (face &rest props)
...
(ert-deftest agent-repl-test-apply-vterm-font-scale-noop-at-one ()
  "apply-vterm-font-scale should be a no-op when the scale is exactly 1.0."
  (agent-repl-test--with-temp-buffer " *test-font-scale-noop*"
    (let ((remapped nil)
          (agent-repl-vterm-font-scale 1.0))
      (cl-letf (((symbol-function 'face-remap-add-relative)
                 (lambda (face &rest props)
                   (push (list face props) remapped))))
        (agent-repl--apply-vterm-font-scale)
...
(ert-deftest agent-repl-test-default-bg-request-index-neg1-no-props ()
  "default-background-request-p with index -1 and no special props should be non-nil."
  (should (agent-repl--default-background-request-p -1 nil)))

(ert-deftest agent-repl-test-default-bg-request-index-neg1-foreground ()
  "default-background-request-p with :foreground should return nil."
  (should-not (agent-repl--default-background-request-p -1 '(:foreground t))))

(ert-deftest agent-repl-test-default-bg-request-index-neg1-inverse-video ()
  "default-background-request-p with :inverse-video should return nil."
  (should-not (agent-repl--default-background-request-p -1 '(:inverse-video t))))

(ert-deftest agent-repl-test-default-bg-request-index-0 ()
  "default-background-request-p with index 0 should return nil."
  (should-not (agent-repl--default-background-request-p 0 nil)))

(ert-deftest agent-repl-test-default-bg-request-index-neg1-both ()
  "default-background-request-p with both :foreground and :inverse-video should return nil."
  (should-not (agent-repl--default-background-request-p -1 '(:foreground t :inverse-video t))))

(ert-deftest agent-repl-test-default-bg-request-index-neg1-empty-args ()
  "default-background-request-p with index -1 and empty args list should return non-nil."
  (should (agent-repl--default-background-request-p -1 '())))
...
(ert-deftest agent-repl-test-vterm-color-advice-claude-default-bg ()
  ...
  (agent-repl-test--with-temp-buffer "*claude-panel-abcd1234*"
    (let ((result (agent-repl--vterm-color-advice
                   ...
                   -1)))
      (should (equal result (agent-repl--grey-hex agent-repl--vterm-background-grey))))))
...
(ert-deftest agent-repl-test-vterm-color-advice-calls-original ()
  "vterm-color-advice should call the original function."
  (let ((original-called nil))
    (agent-repl-test--with-temp-buffer "*scratch*"
      (agent-repl--vterm-color-advice
  ...
  (agent-repl-test--with-temp-buffer "*claude-panel-abcd1234*"
    (let ((agent-repl-debug nil)
      ...
      (cl-letf (((symbol-function 'agent-repl--log-verbose)
        ...
        (agent-repl--vterm-color-advice (lambda (_idx &rest _args) "#ffffff") -1))
      (should (= log-calls 0)))))
