...
(defun agent-repl--memory-state-format-value (val)
...
running every value through `agent-repl--memory-state-format-value'."
  (let ((out (list :ws ws
    ...
    (while pl
      (let ((k (pop pl))
            (v (pop pl)))
        (setq out (nconc out (list k (agent-repl--memory-state-format-value v))))))
    out))

(defun agent-repl--memory-state-write-file (file data)
...
caller (matching `agent-repl--state-save')."
  (when (and ws (boundp 'agent-repl--workspaces))
       ...
       (t
        (agent-repl--with-error-logging "memory-state-save"
          (agent-repl--memory-state-write-file
           file (agent-repl--memory-state-render ws plist))
          (agent-repl--log
