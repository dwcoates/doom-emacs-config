...
INPUT=$(cat)
echo "$(date '+%H:%M:%S.%3N') [subagent_stop_$$] raw_input=$INPUT" >> "$LOGFILE"
CWD=$(echo "$INPUT" | jq -r '.cwd')
SESSION_ID=$(echo "$INPUT" | jq -r '.session_id // empty')
echo "$(date '+%H:%M:%S.%3N') [subagent_stop_$$] parsed_cwd=$CWD session_id=$SESSION_ID" >> "$LOGFILE"
...
printf '%s\n%s\n%s\n' "$CWD" "$SESSION_ID" "$OWNED" > ${AGENT_REPL_STATE_DIR:-$HOME/.claude-emacs}/workspace-notifications/subagent_stop_$$
