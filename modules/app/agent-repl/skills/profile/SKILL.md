...
[
  {"type": "eval",
   "code": "(agent-repl--profile-stop-and-write-file)",
   "workspace": "$ws",
   "note": "profile stop"}
]
EOF
...
The stop dispatch's eval result arrives as a follow-up user message carrying the report file path returned by `agent-repl--profile-stop-and-write-file` (or nil if the profiler was not running). When that arrives:
...
- Call out anything in `modules/app/agent-repl/` specifically — that's the primary target audience.
