...
describe("encodeFrontendCommand — paintAck", () => {
  it("encodes the painted seq as a protojson uint64 string", () => {
    const w = wire({
      requestId: "r1",
      workspace: "ws",
      body: { case: "paintAck", throughSeq: 42, stateGeneration: 5, outcome: "painted" },
    });
    expect(w.paintAck).toEqual({
      throughSeq: "42",
      stateGeneration: "5",
      outcome: "PAINT_OUTCOME_PAINTED",
    });
  });
  ...
  it("encodes an empty-history attestation at seq 0", () => {
    ...
    const w = wire({
      requestId: "r1",
      workspace: "ws",
      body: { case: "paintAck", throughSeq: 0, stateGeneration: 1, outcome: "painted" },
    });
    expect(w.paintAck).toEqual({
      throughSeq: "0",
      stateGeneration: "1",
  ...
  it("encodes the suspended outcome a webview that cannot draw sends", () => {
    const w = wire({
      requestId: "r1",
      workspace: "ws",
      body: { case: "paintAck", throughSeq: 9, stateGeneration: 4, outcome: "suspended" },
    });
    expect((w.paintAck as Record<string, unknown>).outcome).toBe("PAINT_OUTCOME_SUSPENDED");
  });
  ...
  it("carries the workspace the attestation is about", () => {
    const w = wire({
      requestId: "r1",
      workspace: "ws-7",
      body: { case: "paintAck", throughSeq: 3, stateGeneration: 1, outcome: "painted" },
    });
