/**
 * Minimal, dependency-free markdown → HTML renderer for TextStream
 * blocks. Escape-first: every character of source text is HTML-escaped
 * before any markup is emitted, so model output can never inject
 * markup. Supports the common subset: headings, fenced code, inline
 * code, bold, italic, links (http/https only), unordered + ordered
...
export function renderMarkdown(src: string): string {
  ...
  const flushFence = (): void => {
    if (fence !== null) {
      const langClass = fence.lang === "" ? "" : ` class="lang-${escapeHtml(fence.lang)}"`;
      out.push(`<pre class="md-code"><code${langClass}>${escapeHtml(fence.lines.join("\n"))}</code></pre>`);
      fence = null;
    }
  };
