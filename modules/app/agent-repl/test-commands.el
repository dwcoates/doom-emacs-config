...
(ert-deftest agent-repl-cmd-test-explain-config-build-input/embeds-raw-prompt ()
...
(ert-deftest agent-repl-cmd-test-explain-config-build-input/preamble-marks-read-only ()
...
(ert-deftest agent-repl-cmd-test-explain-config-build-input/preamble-instructs-refusal-on-misuse ()
...
(ert-deftest agent-repl-cmd-test-explain-config-dir/defaults-to-doom-config ()
...
(ert-deftest agent-repl-cmd-test-explain-config/empty-prompt-errors ()
  "explain-config signals user-error for an empty prompt."
  (cl-letf (((symbol-function '+workspace-current-name) (lambda () "ws"))
            ((symbol-function 'agent-repl--explain-config-spawn)
             (lambda (_p) 'fake-proc)))
    (should-error (agent-repl-explain-config "") :type 'user-error)))

(ert-deftest agent-repl-cmd-test-explain-config/whitespace-prompt-errors ()
...
(ert-deftest agent-repl-cmd-test-explain-config/forwards-trimmed-prompt ()
...
(ert-deftest agent-repl-cmd-test-explain-config-spawn/runs-in-configured-dir ()
...
(ert-deftest agent-repl-cmd-test-explain-config-spawn/sends-wrapped-input-on-stdin ()
...
(ert-deftest agent-repl-cmd-test-explain-config-format-header/box-drawing-rule ()
  "Header uses unicode box-drawing characters for the top rule, not ASCII dashes."
  (let ((header (agent-repl--explain-config-format-header "anything")))
    (should (string-match-p "━" header))
    (should-not (string-match-p "^--- response" header))))

(ert-deftest agent-repl-cmd-test-explain-config-format-footer/success-uses-check-emoji ()
  "Footer for status=0 includes the check emoji and a success phrase."
  (let ((footer (agent-repl--explain-config-format-footer 0)))
    (should (string-match-p "✅" footer))
    (should (string-match-p "exited cleanly" footer))
    (should (string-match-p "status 0" footer))))

(ert-deftest agent-repl-cmd-test-explain-config-format-footer/failure-uses-cross-emoji ()
  "Footer for non-zero status uses the cross emoji and reports the code."
  (let ((footer (agent-repl--explain-config-format-footer 137)))
    (should (string-match-p "❌" footer))
    (should (string-match-p "exited with errors" footer))
    (should (string-match-p "status 137" footer))))

(ert-deftest agent-repl-cmd-test-explain-config-format-footer/success-and-failure-have-distinct-colors ()
  "Success and failure footers carry distinct foreground colors."
  (let* ((ok (agent-repl--explain-config-format-footer 0))
         (bad (agent-repl--explain-config-format-footer 1))
...
(ert-deftest agent-repl-cmd-test-explain-config-sentinel/inserts-rich-footer-on-exit ()
  "Sentinel inserts the rich footer (emoji + status) when the process exits."
  (let* ((agent-repl-explain-config-buffer-name " *test-explain-config-footer*")
         (buf (get-buffer-create agent-repl-explain-config-buffer-name))
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'process-status) (lambda (_p) 'exit))
          ...
          (agent-repl--explain-config-sentinel proc "finished\n")
          (with-current-buffer buf
...
(ert-deftest agent-repl-cmd-test-explain-config-display-action/uses-right-side-window ()
...
(ert-deftest agent-repl-cmd-test-explain-config-display-action/sits-at-right-slot-0 ()
  "Explain-config window lives at slot 0 on the right side."
  (should (= (cdr (assq 'slot agent-repl--explain-config-display-action))
             0)))

(ert-deftest agent-repl-cmd-test-explain-config-display-action/configures-window-width ()
...
(ert-deftest agent-repl-cmd-test-explain-config-width-fraction/defcustom-defaults-half-frame ()
...
(ert-deftest agent-repl-cmd-test-explain-config-window-width/computes-from-fraction ()
...
(ert-deftest agent-repl-cmd-test-explain-config-window-width/clamps-floor-at-1 ()
...
(ert-deftest agent-repl-cmd-test-explain-config-show/no-op-when-buffer-missing ()
...
(ert-deftest agent-repl-cmd-test-explain-config-show/sets-global-visible-flag ()
...
(ert-deftest agent-repl-cmd-test-explain-config-show/reapplies-width-when-visible ()
...
(ert-deftest agent-repl-cmd-test-explain-config-show/does-not-call-drawer-hide ()
  "Show must NOT call `agent-repl-drawer-hide' regardless of drawer state —
...
(ert-deftest agent-repl-cmd-test-explain-config-show/records-replaced-window-on-takeover ()
...
(ert-deftest agent-repl-cmd-test-explain-config-hide/deletes-windows-for-buffer ()
...
(ert-deftest agent-repl-cmd-test-explain-config-hide/no-op-when-buffer-missing ()
...
(ert-deftest agent-repl-cmd-test-explain-config-hide/clears-global-visible-flag ()
...
(ert-deftest agent-repl-cmd-test-explain-config-hide/does-not-call-drawer-show ()
...
(ert-deftest agent-repl-cmd-test-explain-config-hide/skips-restore-when-window-dead ()
...
(ert-deftest agent-repl-cmd-test-explain-config-close/is-interactive ()
  "Public close command is interactively callable (has interactive form)."
  (should (commandp #'agent-repl-explain-config-close)))

(ert-deftest agent-repl-cmd-test-explain-config-close/delegates-to-hide ()
...
(ert-deftest agent-repl-cmd-test-explain-config-close/clears-flag-and-deletes-windows ()
...
(ert-deftest agent-repl-cmd-test-explain-config-mode/q-binds-to-close ()
  "Minor-mode keymap binds `q' to the public close command."
  (should (eq (lookup-key agent-repl-explain-config-mode-map (kbd "q"))
              #'agent-repl-explain-config-close)))

(ert-deftest agent-repl-cmd-test-explain-config-install-evil-bindings/binds-q-in-normal-and-motion ()
    ...
    (cl-letf (((symbol-function 'evil-define-key)
      ...
      (should (cl-some (lambda (call)
                         (and (equal (nth 0 call) '(normal motion))
                              (eq    (nth 1 call) agent-repl-explain-config-mode-map)
                              (equal (nth 2 call) "q")
                              (eq    (nth 3 call) #'agent-repl-explain-config-close)))
                       calls)))))

(ert-deftest agent-repl-cmd-test-explain-config-install-evil-bindings/blocks-each-insert-entry-key ()
...
(ert-deftest agent-repl-cmd-test-explain-config-install-evil-bindings/insert-entry-key-list-is-canonical ()
  "Insert-entry key list covers the standard evil normal->insert transitions."
  (should (equal agent-repl--explain-config-insert-entry-keys
                 '("i" "I" "a" "A" "o" "O" "s" "S" "c" "C" "R"))))

(ert-deftest agent-repl-cmd-test-explain-config-mode/forces-motion-state-on-enable ()
...
(ert-deftest agent-repl-cmd-test-explain-config-mode/no-motion-state-on-disable ()
...
(ert-deftest agent-repl-cmd-test-explain-config-mode/no-error-when-evil-absent ()
...
`evil-motion-state', which would explode in any non-evil session."
  (let ((real-fboundp (symbol-function 'fboundp)))
    (cl-letf (((symbol-function 'fboundp)
      ...
      (with-temp-buffer
        (agent-repl-explain-config-mode 1)
        (should agent-repl-explain-config-mode)))))

(ert-deftest agent-repl-cmd-test-explain-config-init-buffer/enables-minor-mode ()
  "init-buffer activates `agent-repl-explain-config-mode' in the output buffer."
  (let* ((agent-repl-explain-config-buffer-name " *test-explain-config-mode*")
         (buf (get-buffer-create agent-repl-explain-config-buffer-name)))
    (unwind-protect
        (progn
          (agent-repl--explain-config-init-buffer "any question")
          (with-current-buffer buf
            (should agent-repl-explain-config-mode)))
      (when (buffer-live-p buf) (kill-buffer buf)))))
...
(ert-deftest agent-repl-cmd-test-explain-config-ensure-visible/drops-stale-replaced-window ()
...
before re-showing — the cell belongs to the persp we left, not the new one."
  (let* ((agent-repl-explain-config-buffer-name " *test-explain-persp-stale*")
         (buf (get-buffer-create agent-repl-explain-config-buffer-name))
         (agent-repl--explain-config-global-visible-p t)
         (agent-repl--explain-config-replaced-window (cons 'dead-win 'old-prev)))
    (unwind-protect
        (cl-letf (((symbol-function 'get-buffer-window) (lambda (&rest _) nil))
...
(ert-deftest agent-repl-cmd-test-explain-config-ensure-visible/hides-when-flag-nil-and-shown ()
...
(ert-deftest agent-repl-cmd-test-explain-config-ensure-visible/noop-when-flag-set-and-already-visible ()
  "Persp-reconciliation skips display when flag is set and window is already live."
  (let* ((agent-repl-explain-config-buffer-name " *test-explain-persp-noop*")
         (buf (get-buffer-create agent-repl-explain-config-buffer-name))
         (agent-repl--explain-config-global-visible-p t))
    (unwind-protect
        (cl-letf (((symbol-function 'get-buffer-window)
...
(ert-deftest agent-repl-cmd-test-explain-config-spawn/does-not-open-drawer ()
...
(ert-deftest agent-repl-cmd-test-explain-config-spawn/does-not-call-show ()
...
(ert-deftest agent-repl-cmd-test-explain-config-filter/calls-show-on-chunk ()
...
appears the moment the model starts streaming output."
  (let* ((agent-repl-explain-config-buffer-name " *test-explain-filter-show*")
         (buf (get-buffer-create agent-repl-explain-config-buffer-name))
         (show-called nil))
    (unwind-protect
        (cl-letf (((symbol-function 'process-buffer) (lambda (_p) buf))
                  ((symbol-function 'agent-repl--explain-config-parse-chunk) #'ignore)
                  ((symbol-function 'agent-repl--explain-config-show)
                   (lambda () (setq show-called t))))
          (agent-repl--explain-config-filter 'fake-proc "hello")
          (should show-called))
      (when (buffer-live-p buf) (kill-buffer buf)))))

(ert-deftest agent-repl-cmd-test-explain-config-filter/parses-before-showing ()
  "Filter parses the chunk first, then calls `--show' — content is in the
buffer before the window is displayed."
  (let* ((agent-repl-explain-config-buffer-name " *test-explain-filter-order*")
         (buf (get-buffer-create agent-repl-explain-config-buffer-name))
         (order nil))
    (unwind-protect
        (cl-letf (((symbol-function 'process-buffer) (lambda (_p) buf))
                  ((symbol-function 'agent-repl--explain-config-parse-chunk)
...
;;;; ---- agent-repl explain-config face-markup rendering ----

(defmacro agent-repl-test--with-explain-config-buf (buf-sym &rest body)
  "Bind BUF-SYM to a fresh, parser-initialized explain-config buffer.
Clean up the buffer after BODY runs."
  (declare (indent 1))
  `(let* ((agent-repl-explain-config-buffer-name
           (format " *test-explain-config-render-%s*" (cl-gensym)))
          (,buf-sym (get-buffer-create agent-repl-explain-config-buffer-name)))
     (unwind-protect
         (progn
...
(ert-deftest agent-repl-cmd-test-explain-config-preamble/instructs-face-markup ()
...
(ert-deftest agent-repl-cmd-test-explain-config-preamble/documents-raw-face-escape-hatch ()
...
(ert-deftest agent-repl-cmd-test-explain-config-preamble/prefers-semantic-tags ()
  "Preamble tells the model to prefer semantic tags over the escape hatch."
  (let ((rendered (agent-repl--explain-config-build-input "anything")))
    (should (string-match-p "PREFER semantic" rendered))))

(ert-deftest agent-repl-cmd-test-explain-config-parse/plain-text-passes-through ()
  "Untagged text is inserted verbatim with no face property."
  (agent-repl-test--with-explain-config-buf buf
    (agent-repl--explain-config-parse-chunk buf "hello world")
    (with-current-buffer buf
      (should (equal (buffer-string) "hello world"))
      (should-not (get-text-property 1 'face)))))

(ert-deftest agent-repl-cmd-test-explain-config-parse/single-tag-applies-face ()
  "An ⟦h1⟧…⟦/h1⟧ segment renders with the h1 face and strips the tags."
  (agent-repl-test--with-explain-config-buf buf
    (agent-repl--explain-config-parse-chunk buf "⟦h1⟧Title⟦/h1⟧")
    (with-current-buffer buf
      (should (equal (buffer-string) "Title"))
      (should (eq (get-text-property 1 'face)
                  'agent-repl-explain-config-h1)))))

(ert-deftest agent-repl-cmd-test-explain-config-parse/nested-tags-stack-faces ()
  "Nested tags produce a list face value with innermost first."
  (agent-repl-test--with-explain-config-buf buf
    (agent-repl--explain-config-parse-chunk buf "⟦h1⟧A ⟦b⟧B⟦/b⟧ C⟦/h1⟧")
    (with-current-buffer buf
      (let* ((s (buffer-string))
        ...
        (should (eq a-face 'agent-repl-explain-config-h1))
        (should (equal b-face
                       '(agent-repl-explain-config-bold
                         agent-repl-explain-config-h1)))))))

(ert-deftest agent-repl-cmd-test-explain-config-parse/partial-tag-across-chunks ()
  "An open tag split across two chunks still styles the inner text correctly."
  (agent-repl-test--with-explain-config-buf buf
    (agent-repl--explain-config-parse-chunk buf "pre ⟦b")
    (with-current-buffer buf
      (should (equal (buffer-string) "pre "))
      (should (equal agent-repl--explain-config-pending "⟦b")))
    (agent-repl--explain-config-parse-chunk buf "⟧bold⟦/b⟧ post")
    (with-current-buffer buf
      (let* ((s (buffer-string))
...
(ert-deftest agent-repl-cmd-test-explain-config-parse/partial-close-tag-across-chunks ()
  "A close tag split across two chunks doesn't leak the partial bytes."
  (agent-repl-test--with-explain-config-buf buf
    (agent-repl--explain-config-parse-chunk buf "⟦b⟧bold⟦/b")
    (with-current-buffer buf
      (should (equal (buffer-string) "bold"))
      (should (equal agent-repl--explain-config-pending "⟦/b")))
    (agent-repl--explain-config-parse-chunk buf "⟧ tail")
    (with-current-buffer buf
      (let* ((s (buffer-string))
             (tail-pos (string-match " tail" s)))
        (should (equal s "bold tail"))
        (should (equal agent-repl--explain-config-pending ""))
        (should-not (get-text-property tail-pos 'face s))))))

(ert-deftest agent-repl-cmd-test-explain-config-parse/unknown-tag-emits-verbatim ()
  "Unknown tag names are rendered verbatim with no face change."
  (agent-repl-test--with-explain-config-buf buf
    (agent-repl--explain-config-parse-chunk buf "x ⟦bogus⟧y⟦/bogus⟧ z")
    (with-current-buffer buf
      (should (equal (buffer-string) "x ⟦bogus⟧y⟦/bogus⟧ z"))
      (should-not (get-text-property 1 'face)))))

(ert-deftest agent-repl-cmd-test-explain-config-parse/stray-close-tag-is-noop ()
  "A close tag with no matching open is a no-op on the face stack."
  (agent-repl-test--with-explain-config-buf buf
    (agent-repl--explain-config-parse-chunk buf "before⟦/b⟧after")
    (with-current-buffer buf
      (should (equal (buffer-string) "beforeafter"))
      (should (null agent-repl--explain-config-face-stack))
      (should-not (get-text-property 1 'face)))))

(ert-deftest agent-repl-cmd-test-explain-config-parse/block-and-kbd-have-distinct-faces ()
  "Different tag names map to different defined faces."
  (agent-repl-test--with-explain-config-buf buf
    (agent-repl--explain-config-parse-chunk
    ...
    (with-current-buffer buf
      (let* ((s (buffer-string))
        ...
        (should (eq (get-text-property spc-pos 'face s)
                    'agent-repl-explain-config-kbd))
        (should (eq (get-text-property sym-pos 'face s)
                    'agent-repl-explain-config-sym))))))

;;;; ---- agent-repl explain-config raw-face escape hatch ----

(ert-deftest agent-repl-cmd-test-explain-config-parse/raw-face-applies-plist ()
...
(ert-deftest agent-repl-cmd-test-explain-config-parse/raw-face-nested-inside-semantic ()
...
(ert-deftest agent-repl-cmd-test-explain-config-parse/raw-face-malformed-plist-verbatim ()
...
(ert-deftest agent-repl-cmd-test-explain-config-parse/raw-face-unknown-attr-rejected ()
...
(ert-deftest agent-repl-cmd-test-explain-config-parse/raw-face-partial-across-chunks ()
  "A ⟦face PLIST⟧ split across chunks buffers until the closing ⟧ arrives."
  (agent-repl-test--with-explain-config-buf buf
    (agent-repl--explain-config-parse-chunk buf "before ⟦face :foreground ")
    (with-current-buffer buf
      (should (equal (buffer-string) "before "))
      (should (equal agent-repl--explain-config-pending
                     "⟦face :foreground ")))
    (agent-repl--explain-config-parse-chunk buf "\"red\"⟧hot⟦/face⟧ tail")
    (with-current-buffer buf
      (let* ((s (buffer-string))
...
(ert-deftest agent-repl-cmd-test-explain-config-parse/raw-face-stray-close-verbatim ()
  "A ⟦/face⟧ with no preceding open emits verbatim, stack untouched."
  (agent-repl-test--with-explain-config-buf buf
    (agent-repl--explain-config-parse-chunk buf "x⟦/face⟧y")
    (with-current-buffer buf
      (should (equal (buffer-string) "x⟦/face⟧y"))
      (should (null agent-repl--explain-config-face-stack)))))

(ert-deftest agent-repl-cmd-test-explain-config-parse/raw-face-close-pops-only-plist-top ()
  "⟦/face⟧ pops only when the stack top is a plist, not a face symbol."
  (agent-repl-test--with-explain-config-buf buf
    ;; ⟦b⟧ pushes the bold face symbol; ⟦/face⟧ must NOT pop it.
    (agent-repl--explain-config-parse-chunk buf "⟦b⟧still⟦/face⟧bold⟦/b⟧")
    (with-current-buffer buf
      (let* ((s (buffer-string))
             (bold-pos (string-match "bold" s)))
...
(ert-deftest agent-repl-cmd-test-explain-config-parse/raw-face-empty-attrs-verbatim ()
  "⟦face⟧ with no attrs at all is treated as an unparseable plist."
  (agent-repl-test--with-explain-config-buf buf
    (agent-repl--explain-config-parse-chunk buf "⟦face⟧x⟦/face⟧")
    (with-current-buffer buf
      (should (equal (buffer-string) "⟦face⟧x⟦/face⟧"))
      (should-not (get-text-property 1 'face)))))

(ert-deftest agent-repl-cmd-test-explain-config-parse/semantic-tag-with-stray-attrs-verbatim ()
...
(ert-deftest agent-repl-cmd-test-explain-config-parse-face-attrs/valid-plist ()
  "A well-formed whitelisted plist is returned as-is."
  (should (equal (agent-repl--explain-config-parse-face-attrs
                  " :foreground \"red\" :weight bold")
                 '(:foreground "red" :weight bold))))

(ert-deftest agent-repl-cmd-test-explain-config-parse-face-attrs/nil-on-extra-tokens ()
  "Trailing junk after the plist sexp is rejected."
  (should (null (agent-repl--explain-config-parse-face-attrs
                 " :foreground \"red\" extra"))))

(ert-deftest agent-repl-cmd-test-explain-config-parse-face-attrs/nil-on-odd-length ()
  "An odd-length plist (key without value) is rejected."
  (should (null (agent-repl--explain-config-parse-face-attrs
                 " :foreground \"red\" :weight"))))

(ert-deftest agent-repl-cmd-test-explain-config-parse-face-attrs/nil-on-unknown-key ()
  "A plist containing a non-whitelisted key is rejected."
  (should (null (agent-repl--explain-config-parse-face-attrs
                 " :bogus 1"))))

(ert-deftest agent-repl-cmd-test-explain-config-parse-face-attrs/nil-on-empty ()
  "Empty or whitespace-only attrs return nil (no plist)."
  (should (null (agent-repl--explain-config-parse-face-attrs "")))
  (should (null (agent-repl--explain-config-parse-face-attrs " "))))

(ert-deftest agent-repl-cmd-test-explain-config-filter/routes-chunk-through-parser ()
  "Process filter forwards CHUNK to the parser using PROC's buffer."
  (agent-repl-test--with-explain-config-buf buf
    (cl-letf (((symbol-function 'process-buffer) (lambda (_p) buf)))
      (agent-repl--explain-config-filter 'fake-proc "⟦b⟧hi⟦/b⟧")
      (with-current-buffer buf
        (should (equal (buffer-string) "hi"))
        (should (eq (get-text-property 1 'face)
                    'agent-repl-explain-config-bold))))))

(ert-deftest agent-repl-cmd-test-explain-config-filter/dead-buffer-is-noop ()
  "Filter does not throw if PROC's buffer is dead."
  (let ((buf (generate-new-buffer " *tmp-dead*")))
    (kill-buffer buf)
    (cl-letf (((symbol-function 'process-buffer) (lambda (_p) buf)))
      (should (progn (agent-repl--explain-config-filter 'fake-proc "x") t)))))

(ert-deftest agent-repl-cmd-test-explain-config-init-buffer/resets-parser-state ()
  "init-buffer wipes any leftover pending bytes and face stack from prior runs."
  (let* ((agent-repl-explain-config-buffer-name " *test-explain-config-reset*")
         (buf (get-buffer-create agent-repl-explain-config-buffer-name)))
    (unwind-protect
        (progn
          (with-current-buffer buf
            (setq agent-repl--explain-config-pending "⟦h"
                  agent-repl--explain-config-face-stack
                  '(agent-repl-explain-config-h1)))
          (agent-repl--explain-config-init-buffer "q")
          (with-current-buffer buf
            (should (equal agent-repl--explain-config-pending ""))
            (should (null agent-repl--explain-config-face-stack))))
      (when (buffer-live-p buf) (kill-buffer buf)))))

(ert-deftest agent-repl-cmd-test-explain-config-spawn/registers-face-filter ()
  ...
  (let (captured-filter)
    (cl-letf (((symbol-function 'make-process)
              ...
              ((symbol-function 'display-buffer) #'ignore))
      (agent-repl--explain-config-spawn "anything")
      (should (eq captured-filter #'agent-repl--explain-config-filter)))))
