...
(ert-deftest agent-repl-services-test-runtime-refuses-active-turn-before-mutation ()
  ...
  (let (mutated)
    (cl-letf (((symbol-function 'agent-repl--frontend-runtime-bounce-preflight-async)
               (lambda (callback) (funcall callback :responsive)))
  ...
  (let (events)
    (cl-letf (((symbol-function 'agent-repl--frontend-runtime-bounce-preflight-async)
              ...
              ((symbol-function 'agent-repl--frontend-bounce-after-build)
               (lambda (&optional preflight _stop-shims on-complete)
      ...
      (should (eq :pending (agent-repl-runtime-restart)))
...
is the default and the stop is the explicit request."
  (let (stop-shims-seen)
    (cl-letf (((symbol-function 'agent-repl--frontend-runtime-bounce-preflight-async)
              ...
              ((symbol-function 'agent-repl--frontend-bounce-after-build)
               (lambda (&optional _preflight stop-shims on-complete)
  ...
  (let (events)
    (cl-letf (((symbol-function 'agent-repl--frontend-init-inhibited-p) (lambda () nil))
              ((symbol-function 'agent-repl--frontend-runtime-bounce-preflight-async)
               (lambda (callback) (push 'daemon-preflight events) (funcall callback :absent)))
              ((symbol-function 'agent-repl--frontend-all-turn-active-session-ids)
               (lambda () nil))
              ((symbol-function 'agent-repl--shim-services-assert-launchd-loaded)
              ...
              ((symbol-function 'agent-repl--frontend-bounce-after-build)
               (lambda (&optional _preflight _stop-shims on-complete)
                 (push 'daemon-bounce events) (funcall on-complete 'started) t))
              ((symbol-function 'agent-repl--frontend-wait-ready)
               (lambda () (push 'ready events) t))
              ((symbol-function 'agent-repl--frontend-wait-daemon-healthy)
               (lambda () (push 'health events) t))
      ...
      (should (eq :pending (agent-repl--runtime-startup-prepare)))
      (should (equal (nreverse events)
