...
| `agent-repl--setup-worktree-session` | Requires `make-agent-repl-instantiation`, `agent-repl--initialize-claude`. | (1) force-bare-metal=t sets :active-env to :bare-metal, (2) default-directory is bound to path during initialize-claude. |
