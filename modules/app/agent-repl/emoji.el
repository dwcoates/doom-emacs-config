...
wrapper) so the call is mocked by the test-time runtime guards."
  (let ((out (agent-repl--git-string-quiet "rev-parse" "--abbrev-ref" "HEAD")))
...
string would miss commits authored under the new format."
  (let* ((n (or lookback agent-repl-emoji-lookback))
...
without going through `git rev-parse'."
  (let ((branch (or branch-override (agent-repl--current-branch))))
...
recent-emojis list through `--random-commit-emoji')."
  (let ((branch (agent-repl--current-branch)))
...
Tests\")."
  (let ((git-dir (agent-repl--git-string-quiet "rev-parse" "--git-common-dir")))
...
If an existing prepare-commit-msg hook is found, backs it up first."
  (interactive)
  (let ((hooks-dir (agent-repl--git-hooks-dir)))
    (unless hooks-dir
