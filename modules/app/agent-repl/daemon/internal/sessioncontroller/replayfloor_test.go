...
func TestObservingAClearRecordsTheReplayFloor(t *testing.T) {
	...
	// Act.
	h.controller(t).consumer.Consume(clearEvent(30, "u-clear"))
...
func TestObservingACompactionRecordsTheReplayFloor(t *testing.T) {
	...
	// Act.
	h.controller(t).consumer.Consume(compactEvent(44, "b-1"))
...
func TestASeqlessClearDoesNotRaiseTheFloor(t *testing.T) {
	...
	// Act.
	h.controller(t).consumer.Consume(clearEvent(0, "u-clear"))
...
func TestTheFloorSurvivesADaemonRestart(t *testing.T) {
	// Arrange — the whole reason the mark is a durable store rather than controller
	...
	first.controller(t).consumer.Consume(clearEvent(300, "u-clear"))
	first.m.Close()
...
func TestAManagerWithoutAClearCompactStoreIsRejected(t *testing.T) {
	// Arrange — a session controller that observed a clear and had nowhere to record it
...
// newRestartHarness builds a session controller over a CALLER-OWNED floor store, so two
