...
// walk-up has no checkout above it to find. Resolving from the package source
// and going through the ordinary DirEnv override means the tests exercise the
// same resolution path a user's own override would.
...
// `mustPrompt(t)(ClassifierPrompt(req))`. It fails the test when a prompt
// could not be composed, so a broken prompt file surfaces as a named failure
// rather than an empty string quietly satisfying a Contains assertion.
