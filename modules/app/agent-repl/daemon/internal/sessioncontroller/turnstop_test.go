...
// newTurnStopRig builds a Manager with no session controllers: turnstop.go's two entry
...
func TestDrainInterruptsALiveTurn(t *testing.T) {
	...
	if !cl.contains(`session-controller: teardown turn drain interrupt DELIVERED ws="ws" session=s1 path=hibernate_session outcome=INTERRUPT_OUTCOME_INTERRUPTED`) {
...
func TestDrainSkipsAWorkspaceWithNoLiveTurn(t *testing.T) {
	...
	if !cl.contains(`session-controller: teardown turn drain SKIPPED ws="ws" session=s1 path=hibernate state=RENDER_STATE_DONE`) {
...
func TestDrainSkipsWhenTheShimConnectionIsAlreadyGone(t *testing.T) {
	...
	applier.setCurrent("ws", thinkingState())
	// Act.
	m.drainLiveTurnForStop("ws", "s1", "session_controller_exit", nil)
	// Assert.
...
func TestDrainReportsAnInterruptThatNeverAnswers(t *testing.T) {
	...
	if !cl.contains(`session-controller: teardown turn drain interrupt FAILED ws="ws" session=s1 path=hibernate_session`) ||
...
func TestStopShimSettlingTurnSynthesizesTheCloseWhenNothingElseDid(t *testing.T) {
	...
	want := staleTurnCloseCall{workspace: "ws", sessionID: "s1", reason: "hibernate_session", soleSessionController: true}
	if len(closes) != 1 || closes[0] != want {
		t.Fatalf("closes = %+v, want exactly %+v", closes, want)
	}
	if !cl.contains(`session-controller: teardown axis close SYNTHESIZED ws="ws" session=s1 path=hibernate_session`) {
...
func TestStopShimSettlingTurnReportsAnHonestCloseDistinguishably(t *testing.T) {
	...
	// Assert.
	if !cl.contains(`session-controller: teardown axis close NOT NEEDED ws="ws" session=s1 path=hibernate`) {
...
func TestStopShimSettlingTurnClosesTheAxisEvenWhenTheStopFails(t *testing.T) {
	...
	if !cl.contains(`session-controller: teardown shim stop FAILED ws="ws" session=s1 path=hibernate_session`) {
...
func TestStopShimSettlingTurnLogsAFailedCloseWithoutFailingTheStop(t *testing.T) {
	...
	if !cl.contains(`session-controller: teardown axis close FAILED ws="ws" session=s1 path=hibernate sole_session_controller=true: state db is gone`) {
...
func TestStopShimSettlingTurnSkipsTheCloseWithNoWorkspace(t *testing.T) {
	...
	if !cl.contains("session-controller: teardown axis close SKIPPED session=s1 path=hibernate_session") {
...
func TestHibernateClosesTheAxisThroughTheFunnel(t *testing.T) {
	...
	// Assert.
	closes := applier.staleTurnClosesApplied()
	want := staleTurnCloseCall{workspace: "ws", sessionID: "s1", reason: "hibernate", soleSessionController: true}
	if len(closes) != 1 || closes[0] != want {
		t.Fatalf("closes = %+v, want exactly %+v", closes, want)
	}
}
...
// workspace passes soleSessionController=false, so it can never spend a claim that may
// belong to the replacement.
func TestSupersededSessionStopNeverClaimsSoleSessionController(t *testing.T) {
	...
	// Assert.
	closes := applier.staleTurnClosesApplied()
	want := staleTurnCloseCall{workspace: "ws", sessionID: "s-old", reason: "hibernate_session_superseded", soleSessionController: false}
	if len(closes) != 1 || closes[0] != want {
		t.Fatalf("closes = %+v, want exactly %+v", closes, want)
	}
}
