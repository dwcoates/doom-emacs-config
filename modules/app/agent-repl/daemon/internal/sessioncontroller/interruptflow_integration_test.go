...
// pins the window's open/clear rules, interrupt_test.go pins the session controller's
...
const (
	...
	// interruptFlowNowMs is the frozen clock both resolvers and the session controller
...
// controller is resolved BEFORE the lock because existing() takes the same mutex.
