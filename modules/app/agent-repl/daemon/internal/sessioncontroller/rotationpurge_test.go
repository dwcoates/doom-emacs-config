...
func (r *rotatingRegistrar) AdoptVendorSessionID(sessionID, csid string) (bool, string, bool) {
	rotated, previous, adopted := r.fakeRegistrar.AdoptVendorSessionID(sessionID, csid)
	if rotated {
...
// is armed at all. The controller is resolved FIRST because existing() takes the
// same lock.
...
func TestRotationResetsTheRetainedSeqCeiling(t *testing.T) {
	...
	// Assert
	if got := h.controller(t).consumer.newestRetainedSeq(); got != 0 {
