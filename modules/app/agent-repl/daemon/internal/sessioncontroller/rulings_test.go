...
type fakeRegistrar struct {
	...
	// currentModel is the value the fake record holds, so the prior value the
	// production registrar reports back is modeled rather than stubbed away.
	currentModel string
...
func TestPersistSessionDeathIsANoOpWithoutARegistrar(t *testing.T) {
	// Arrange: a session controller built without a registrar (a unit harness).
