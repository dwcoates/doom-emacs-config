// Package pprofsurface owns the daemon's OPT-IN, LOCAL-ONLY Go profiling
// surface.
//
...
const EnvAddr = "AGENT_REPL_PPROF_ADDR"
...
// publish the daemon's stacks and heap to the network.
...
func openUnix(path string) (*Surface, error) {
	...
	// daemon that died holding this path leaves a stale socket which would
...
func newSurface(network, address string, listener net.Listener) *Surface {
	return &Surface{
		...
		// registers on the default mux as a side effect, and a daemon that
