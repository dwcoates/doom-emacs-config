...
// newProducerHandler builds a permHandler wired exactly as the driver wires it:
...
func newProducerHandler(t *testing.T) (permHandler, *fakeApplier) {
	...
	cons := newConsumer("ws", "s1", &fakePusher{}, applier, nil, newFakeClearCompactStore(), nil, nil, nil, nil, nil)
