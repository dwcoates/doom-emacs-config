...
// the transcript when it resolves a resume target — so the driver now writes
...
func TestARepeatedAnnouncementIsWrittenOnlyOnce(t *testing.T) {
	...
	// a driver that re-called the registry per event would spray writes.
	reg := &fakeRegistrar{}
	m := newRegistrarManager(t, reg)
