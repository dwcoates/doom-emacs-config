...
func launchSkill(t *testing.T, h *queueHarness) string {
	t.Helper()
	h.driver().consumer.Consume(transcriptToolUseEvent(t, 10, "a-call", "toolu_demo", "Skill"))
	h.driver().consumer.Consume(transcriptToolResultEvent(t, 11, "u-result", "toolu_demo"))
	return "toolu_demo"
}
...
func TestASkillBodyAttachesToItsOwnCall(t *testing.T) {
	...
	// Act
	h.driver().consumer.Consume(transcriptMetaUserEvent(t, 12, "u-body", "u-result", skillBody))
...
func TestOneInvocationYieldsOneSkillBodyItem(t *testing.T) {
	...
	// Act
	h.driver().consumer.Consume(transcriptMetaUserEvent(t, 12, "u-notice", "u-result", "(Re-invocation of /demo — already loaded.)"))
	h.driver().consumer.Consume(transcriptMetaUserEvent(t, 13, "u-body", "u-notice", skillBody))
...
func TestConcurrentSkillsEachGetTheirOwnBody(t *testing.T) {
	...
	h := newQueueHarness(t, nil)
	h.driver().consumer.Consume(transcriptToolUseEvent(t, 10, "a-one", "toolu_one", "Skill"))
	h.driver().consumer.Consume(transcriptToolUseEvent(t, 11, "a-two", "toolu_two", "Skill"))
	h.driver().consumer.Consume(transcriptToolResultEvent(t, 12, "u-res-one", "toolu_one"))
	h.driver().consumer.Consume(transcriptToolResultEvent(t, 13, "u-res-two", "toolu_two"))

	// Act: the SECOND call's body lands first.
	h.driver().consumer.Consume(transcriptMetaUserEvent(t, 14, "u-body-two", "u-res-two", skillBody+" two"))
	h.driver().consumer.Consume(transcriptMetaUserEvent(t, 15, "u-body-one", "u-res-one", skillBody+" one"))
...
func TestANonSkillMetaRecordIsWithheldFromTheFeed(t *testing.T) {
	...
	h := newQueueHarness(t, nil)
	h.driver().consumer.Consume(transcriptToolUseEvent(t, 10, "a-ask", "toolu_ask", "AskUserQuestion"))
	h.driver().consumer.Consume(transcriptToolResultEvent(t, 11, "u-ask-res", "toolu_ask"))

	// Act
	h.driver().consumer.Consume(transcriptMetaUserEvent(t, 12, "u-nudge", "u-ask-res", "Continue from where you left off."))
...
func TestWithholdingANonSkillMetaRecordIsLoud(t *testing.T) {
	...
	// Act
	h.driver().consumer.Consume(transcriptMetaUserEvent(t, 12, "u-nudge", "u-unknown", "Continue from where you left off."))
...
func TestAnUncorrelatableBodyIsWithheldRatherThanGuessed(t *testing.T) {
	...
	h.driver().consumer.Consume(transcriptMetaUserEvent(t, 11, "u-body", "u-result", skillBody))
...
func TestAMetaRecordClaimsNoPromptReceipt(t *testing.T) {
	...
	if got := len(h.driver().consumer.snapshotEchoes()); got != 1 {
...
func TestAReplayedBodyResolvesToTheSameCard(t *testing.T) {
	...
	h.driver().consumer.Consume(body)

	// Act: a re-pull pushes the same record again, not live.
	h.driver().consumer.pushConversation(body, false)
...
func TestAReplayedMetaRecordIsWithheldToo(t *testing.T) {
	...
	h.driver().consumer.pushConversation(nudge, false)
...
func TestAnUnflaggedMachineryRecordIsStillWithheldByItsEnvelope(t *testing.T) {
	...
	// Act
	h.driver().consumer.Consume(transcriptUserEvent(t, 12, "u-machinery", compactMachinery))
...
func TestARealPromptIsNeitherWithheldNorAttached(t *testing.T) {
	...
	// Act
	h.driver().consumer.Consume(transcriptUserEvent(t, 12, "u-real", "carry on"))
