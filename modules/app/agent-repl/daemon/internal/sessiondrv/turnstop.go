...
//
// `drainLiveTurnForStop` runs BEFORE the driver context is cancelled, while the
...
// is an interface rather than *driven so the timeout and the already-dead-shim
...
//
// IT MUST BE CALLED BEFORE THE DRIVER CONTEXT IS CANCELLED. The cancel ends
...
func (m *Manager) drainLiveTurnForStop(workspace, sessionID, path string, cl turnInterrupter) {
	m.logf("sessiondrv: teardown turn drain ENTRY ws=%q session=%s path=%s client_present=%v",
		workspace, sessionID, path, cl != nil)
	if cl == nil {
		m.logf("sessiondrv: teardown turn drain SKIPPED ws=%q session=%s path=%s — there is no live shim connection to interrupt over, so the teardown's own axis close is the only edge available",
	...
	switch {
	case err != nil:
		m.logf("sessiondrv: teardown turn drain state read FAILED ws=%q session=%s path=%s: %v — interrupting anyway, because the round trip is cheap and skipping it on an unreadable log would forfeit the shim's own turn end",
			workspace, sessionID, path, err)
	case !found:
		m.logf("sessiondrv: teardown turn drain SKIPPED ws=%q session=%s path=%s — the log knows nothing about this workspace, so it has no turn to interrupt",
			workspace, sessionID, path)
		return
	case !st.GetTurnActive():
		m.logf("sessiondrv: teardown turn drain SKIPPED ws=%q session=%s path=%s state=%s — no turn is in flight, so there is nothing for the stop to interrupt",
			workspace, sessionID, path, st.GetState())
		return
	default:
		m.logf("sessiondrv: teardown turn drain INTERRUPTING ws=%q session=%s path=%s state=%s — the shim is about to be stopped over a live turn, and its own interrupt is the only way that turn's end gets reported honestly",
			workspace, sessionID, path, st.GetState())
	}
	...
	if err != nil {
		m.logf("sessiondrv: teardown turn drain interrupt FAILED ws=%q session=%s path=%s outcome=%s timeout=%s: %v — the shim will not report this turn's end, so the teardown closes the agent axis itself",
			workspace, sessionID, path, outcome, bound, err)
		return
	}
	m.logf("sessiondrv: teardown turn drain interrupt DELIVERED ws=%q session=%s path=%s outcome=%s",
...
// Spawner.StopShim.
//
// SOLEDRIVER says whether this stop is the workspace's OWN teardown, and it
...
func (m *Manager) stopShimSettlingTurn(workspace, sessionID, path string, soleDriver bool) error {
	m.logf("sessiondrv: teardown shim stop ENTRY ws=%q session=%s path=%s sole_driver=%v",
		workspace, sessionID, path, soleDriver)
	stopErr := m.cfg.Spawner.StopShim(sessionID, m.shimPIDFor(sessionID))
	if stopErr != nil {
		m.logf("sessiondrv: teardown shim stop FAILED ws=%q session=%s path=%s: %v — the agent axis is still closed below, because a stop that failed leaves the turn no more reportable than one that succeeded",
			workspace, sessionID, path, stopErr)
	}
	m.settleTurnAfterStop(workspace, sessionID, path, soleDriver)
	return stopErr
}
...
func (m *Manager) settleTurnAfterStop(workspace, sessionID, path string, soleDriver bool) {
	if workspace == "" {
		m.logf("sessiondrv: teardown axis close SKIPPED session=%s path=%s — the stop named no workspace, so there is no agent axis to close",
			sessionID, path)
		return
	}
	closed, err := m.cfg.SSM.CloseStaleTurn(workspace, sessionID, path, soleDriver)
	if err != nil {
		m.logf("sessiondrv: teardown axis close FAILED ws=%q session=%s path=%s sole_driver=%v: %v — the workspace may stay latched in THINKING until another edge supersedes it",
			workspace, sessionID, path, soleDriver, err)
		return
	}
	if closed {
		m.logf("sessiondrv: teardown axis close SYNTHESIZED ws=%q session=%s path=%s — no `TurnEnded` reached the log before the shim died, so the daemon wrote the closing row itself as the last party that could know the turn is over",
			workspace, sessionID, path)
		return
	}
	m.logf("sessiondrv: teardown axis close NOT NEEDED ws=%q session=%s path=%s — the axis already carried no live turn, so the shim's own end closed it honestly",
		workspace, sessionID, path)
}
