...
func (c *consumer) pushUserEcho(requestID, text string) {
	...
	c.mu.Unlock()
	c.logf("sessiondrv: prompt echo pushed ws=%q session=%s request_id=%s len=%d unclaimed=%d",
		c.workspace, c.sessionID, requestID, len(text), pending)
	c.pushLocalItem(item)
}

// echo pushes a driver's prompt receipt, if the submit has an identity to key
...
func (m *Manager) echo(d *driven, requestID, text string) {
	if requestID == "" {
		return
	}
	...
	for _, it := range cd.GetItems() {
		if !isPromptUserMessage(it) {
			...
			if id := it.GetRequestId(); id != "" && it.GetUserMessage() != nil && c.claimEcho(id) {
				c.logf("sessiondrv: prompt receipt superseded ws=%q session=%s request_id=%s — the durable line arrived carrying the id itself",
		...
		if requestID == "" {
			c.logf("sessiondrv: user turn UNATTRIBUTED ws=%q session=%s uuid=%s — no submit of this daemon's is outstanding for it",
				c.workspace, c.sessionID, it.GetUuid())
			continue
		}
		it.RequestId = requestID
		c.logf("sessiondrv: user turn attributed ws=%q session=%s uuid=%s request_id=%s (the receipt it supersedes is retired)",
