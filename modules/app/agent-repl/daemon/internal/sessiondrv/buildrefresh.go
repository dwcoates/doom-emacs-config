...
func (m *Manager) refreshStaleShim(workspace, sessionID, reported string) bool {
	...
	m.mu.Unlock()
	if already {
		m.logf("sessiondrv: session %s (ws %q) STILL reports shim build %s against current %s after a refresh; NOT bouncing again — the bundle or the stamp is wrong, and a second bounce would loop",
			sessionID, workspace, reported, want)
		return false
	}
	m.logf("sessiondrv: STALE SHIM session=%s ws=%q build=%s current=%s — this shim survived a deploy and is running superseded code; bouncing it onto the current bundle",
		sessionID, workspace, reported, want)
	go func() {
		if err := m.RestartSession(m.rootCtx, workspace); err != nil {
			m.logf("sessiondrv: session %s (ws %q) stale-shim refresh FAILED; the workspace keeps running the superseded bundle: %v",
				sessionID, workspace, err)
			return
		}
		m.logf("sessiondrv: session %s (ws %q) refreshed onto shim build %s", sessionID, workspace, want)
	}()
	return true
}
...
func (m *Manager) noteUnknownBuild(workspace, sessionID, reported, want string) {
	...
	m.logf("sessiondrv: session %s (ws %q) shim build identity is UNKNOWN (shim=%q current=%q); no staleness refresh is possible for it — an unknown identity is not a mismatch",
		sessionID, workspace, reported, want)
}
...
func (m *Manager) RestartSession(ctx context.Context, workspace string) error {
	...
	if !ok {
		return fmt.Errorf("sessiondrv: workspace %q has no session to restart", workspace)
	}
	m.mu.Lock()
	_, live := m.byWS[workspace]
	m.mu.Unlock()
	if live {
		// The full teardown: it cancels the driver, reports the workspace
		...
		// asked to replace.
		return fmt.Errorf("sessiondrv: restarting session %s (ws %q): stopping the orphaned shim: %w", sessionID, workspace, err)
	} else {
		m.logf("sessiondrv: hard restart ws=%q session=%s: no live driver; stopped any orphaned shim", workspace, sessionID)
	}
	if _, err := m.ensure(ctx, workspace); err != nil {
		return fmt.Errorf("sessiondrv: restarting session %s (ws %q): bringing it back up: %w", sessionID, workspace, err)
	}
	m.logf("sessiondrv: hard restart ws=%q session=%s COMPLETE (same conversation, fresh shim)", workspace, sessionID)
	return nil
}
