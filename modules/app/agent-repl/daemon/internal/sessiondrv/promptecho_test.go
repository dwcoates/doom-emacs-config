...
func TestTheDurableLineIsStampedWithTheSubmitItAnswers(t *testing.T) {
	...
	h.driver().consumer.Consume(transcriptUserEvent(t, 12, "u1", "hello there"))
...
func TestAnUnmatchedDurableLineStaysUnattributed(t *testing.T) {
	...
	// Act.
	h.driver().consumer.Consume(transcriptUserEvent(t, 12, "u1", "from somewhere else"))
...
func TestTwoOutstandingSubmitsAreAttributedOldestFirst(t *testing.T) {
	...
	h.driver().consumer.Consume(transcriptUserEvent(t, 12, "u1", "first"))
...
func TestAReplayedUserLineClaimsNoReceipt(t *testing.T) {
	...
	h.driver().consumer.pushConversation(transcriptUserEvent(t, 3, "old", "ancient prompt"), false)
...
func TestARaisedReplayFloorDropsTheOutstandingReceipts(t *testing.T) {
	...
	h.driver().consumer.Consume(&corev1.Event{
	...
	if got := len(h.driver().consumer.snapshotEchoes()); got != 0 {
