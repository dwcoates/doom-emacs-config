...
func (c *consumer) curateMetaRecords(cd *frontendv1.ConversationDelta, envs map[string]frontend.RecordEnvelope) {
	...
	for _, it := range items {
		...
		if toolUseID != "" && isSkillBodyText(text) {
			c.logf("sessiondrv: skill body ATTACHED to its card ws=%q session=%s seq=%d uuid=%s tool_use_id=%s len=%d",
		...
		c.logf("sessiondrv: user turn WITHHELD as harness meta record ws=%q session=%s seq=%d uuid=%s parent=%s skill=%q head=%q — the harness flagged this record isMeta, meaning it wrote it FOR THE MODEL rather than a person typing it; the store keeps it, the conversation feed does not",
