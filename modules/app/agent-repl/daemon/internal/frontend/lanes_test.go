...
func TestCommandLanesRunEveryReadCommandBeforeCloseReturns(t *testing.T) {
	...
	release := make(chan struct{})
	lanes := newCommandLanes(testLogf(t), func(cmd *frontendv1.FrontendCommand) {
		mu.Lock()
	...
	// Act.
	lanes.submit(openCmd("r1", "/ws/a"))
	<-started
	lanes.submit(openCmd("r2", "/ws/a"))
	close(release)
	lanes.close()
...
func TestCommandLanesFailHardOnASubmitAfterClose(t *testing.T) {
	...
	lanes := newCommandLanes(
		...
		func(cmd *frontendv1.FrontendCommand) { ran = append(ran, cmd.GetRequestId()) },
	)
	lanes.close()
	...
	recovered := func() (v any) {
		...
		lanes.submit(openCmd("late", "/ws/a"))
		return nil
	}()
