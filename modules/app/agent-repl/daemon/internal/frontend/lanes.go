...
// ---------------------------------------------------------------------------
// What is NOT on a lane
// ---------------------------------------------------------------------------
//
// client_log used to ride the workspace's lane, then its low-priority sublane.
// It rides neither now: it is acked at ingress and written by the connection's
// telemetry writer (telemetry.go), because nothing waits on its outcome and a
// flood of it has no business occupying an executor that interactive commands
// share. Every command that reaches a lane is one whose completion something
// waits for.
