...
type commandLanes struct {
	...
	// different lanes.
	run func(*frontendv1.FrontendCommand)
...
type commandLane struct {
	...
	mu     sync.Mutex
	queue  []*frontendv1.FrontendCommand
	closed bool
	// ready is a coalescing wakeup (capacity 1), never the queue itself.
	ready chan struct{}
}

func newCommandLanes(logf dlog.Logf, run func(*frontendv1.FrontendCommand)) *commandLanes {
...
func (l *commandLanes) submit(cmd *frontendv1.FrontendCommand) {
	key := laneKey(cmd)
	l.mu.Lock()
	if l.closed {
		l.mu.Unlock()
...
func (lane *commandLane) push(cmd *frontendv1.FrontendCommand) {
	lane.mu.Lock()
	lane.queue = append(lane.queue, cmd)
	lane.mu.Unlock()
	lane.signal()
}
...
func (lane *commandLane) next() (cmd *frontendv1.FrontendCommand, ok bool, done bool) {
	lane.mu.Lock()
