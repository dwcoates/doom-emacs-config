...
func New(opt Options, log Logf) *Tracker {
	...
	return &Tracker{tasks: map[string]*task{}, opt: opt, log: log}
}
...
func (t *Tracker) Activity(id string, nowMs int64) {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *Tracker) MarkVanished(id string, nowMs int64) {
	t.mu.Lock()
	defer t.mu.Unlock()
	if tk, ok := t.tasks[id]; ok && tk.vanishedAtMs == 0 {
		tk.vanishedAtMs = nowMs
	}
}

// MarkPresent clears a vanish timer (the file reappeared before grace elapsed).
func (t *Tracker) MarkPresent(id string) {
	t.mu.Lock()
	defer t.mu.Unlock()
	if tk, ok := t.tasks[id]; ok {
		tk.vanishedAtMs = 0
