...
(the `shim/` directory has no local `node_modules`; `shim/package.json` declares `^0.1.0` and
`shim/package-lock.json` pins `0.1.77`). Transcript evidence is from ~60 real
...
repository.** `grep -rl "tool_use_result\|toolUseResult" shim/src daemon/internal webapp/src shared/`
returns nothing.
...
`shim/src/session.ts:497-532` (`mapUserMessage`), which walks `msg.message.content` for
...
1. **SDK.** `realQueryOptions` sets `includePartialMessages: true` (`shim/src/main.ts:128`). Every
