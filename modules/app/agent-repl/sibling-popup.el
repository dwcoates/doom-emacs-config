...
(defun agent-repl-sibling-popup--popup-height (target alist)
...
(defun agent-repl-sibling-popup--fallback (buffer alist)
...
vterm popup rule for the canonical caller."
  (let ((target (agent-repl-sibling-popup--target-window)))
    (if (window-live-p target)
        (let* ((height-lines (agent-repl-sibling-popup--popup-height target alist))
               (new-win (split-window target (- height-lines) 'below)))
          (set-window-buffer new-win buffer)
          new-win)
      (agent-repl-sibling-popup--fallback buffer alist))))
