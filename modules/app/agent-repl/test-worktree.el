...
;;;; ---- Tests: handle-merge-command routes to the daemon request ----

(ert-deftest agent-repl-test-handle-merge-command-merges-resolved-workspace ()
  "A resolvable command name reaches the async entry point."
  (agent-repl-test--with-clean-state
    (agent-repl--ws-put "foo" :project-dir "/tmp/foo")
    (let (merged)
      (cl-letf (((symbol-function 'agent-repl--workspace-merge-async)
                 (lambda (ws) (setq merged ws))))
        (agent-repl--handle-merge-command '((workspace . "foo")))
        (should (equal merged "foo"))))))

(ert-deftest agent-repl-test-handle-merge-command-passes-only-the-workspace ()
  "The handler forwards a workspace and nothing else — no repo root, no mode."
  (agent-repl-test--with-clean-state
    (agent-repl--ws-put "foo" :project-dir "/tmp/foo")
    (let (args)
      (cl-letf (((symbol-function 'agent-repl--workspace-merge-async)
        ...
        (agent-repl--handle-merge-command
         '((workspace . "foo") (pr_was_merged . t)))
        (should (equal args '("foo")))))))

(ert-deftest agent-repl-test-handle-merge-command-resolves-by-project-dir ()
  "`project_dir' resolves the workspace name the request is keyed by."
  (agent-repl-test--with-clean-state
    (let ((dir (make-temp-file "agent-repl-merge-cmd-" t)))
      (unwind-protect
          (let (merged)
            (agent-repl--ws-put "by-dir" :project-dir dir)
            (cl-letf (((symbol-function 'agent-repl--workspace-merge-async)
                       (lambda (ws) (setq merged ws))))
              (agent-repl--handle-merge-command
               `((workspace . "unknown") (project_dir . ,dir)))
              (should (equal merged "by-dir"))))
        (delete-directory dir t)))))

(ert-deftest agent-repl-test-handle-merge-command-defers-for-before-ws-merge ()
  "A pending `before_ws_merge' action defers the merge instead of running it."
  (agent-repl-test--with-clean-state
    (agent-repl--ws-put "foo" :project-dir "/tmp/foo")
    (let (merged)
      (cl-letf (((symbol-function 'agent-repl--maybe-run-before-ws-merge-prompt)
                 (lambda (_ws) t))
...
(ert-deftest agent-repl-test-handle-merge-command-skips-unknown-workspace ()
  "An unresolvable workspace issues no merge request."
  (agent-repl-test--with-clean-state
    (let (merged)
      (cl-letf (((symbol-function 'agent-repl--workspace-merge-async)
                 (lambda (ws) (setq merged ws))))
        (agent-repl--handle-merge-command '((workspace . "nope")))
        (should-not merged)))))
