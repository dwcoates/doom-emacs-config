...
(declare-function agent-repl--frontend-webview-execute-script-value
...
(defcustom agent-repl-recovery-slo-poll-ms 250
...
this cadence exists for the page, which can only be ASKED.  Fine enough
that the recorded gap is accurate to a quarter second against a three
second budget, coarse enough that a bounce with a dozen workspaces is
a dozen script evaluations per tick rather than hundreds."
  :type 'integer
  :group 'agent-repl)
...
(defun agent-repl--recovery-slo-probe-script ()
  "Return the JS asking the page for its recovery report.
    ...
    (when (buffer-live-p buf)
      (condition-case err
          (agent-repl--frontend-webview-execute-script-value
           buf (agent-repl--recovery-slo-probe-script)
           (lambda (raw)
             (when (agent-repl--recovery-slo-probe-satisfied-p raw)
